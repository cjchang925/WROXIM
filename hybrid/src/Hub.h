...
SC_MODULE(Hub) {
  ...
  void setFlitTransmissionCycles(int cycles, int ch_id) {
    flit_transmission_cycles[ch_id] = cycles;
  }
  ...
  Hub(sc_module_name nm, int id, TokenRing *tr) : sc_module(nm) {
    ...
    for (int i = 0; i < num_ports; i++) {
      buffer_from_tile[i].SetMaxBufferSize(
          GlobalParams::hub_configuration[local_id].fromTileBufferSize);
      buffer_to_tile[i].SetMaxBufferSize(
          GlobalParams::hub_configuration[local_id].toTileBufferSize);
    }
    ...
    for (unsigned int i = 0; i < txChannels.size(); i++) {
      ...
      init[txChannels[i]]->buffer_tx.SetMaxBufferSize(
          GlobalParams::hub_configuration[local_id].txBufferSize);
      ...
      token_ring->attachHub(
          txChannels[i], local_id, current_token_holder[txChannels[i]],
          current_token_expiration[txChannels[i]], flag[txChannels[i]]);
    ...
    for (unsigned int i = 0; i < rxChannels.size(); i++) {
      ...
      target[rxChannels[i]]->buffer_rx.SetMaxBufferSize(
          GlobalParams::hub_configuration[local_id].rxBufferSize);
    }
