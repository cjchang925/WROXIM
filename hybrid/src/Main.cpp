...
int sc_main(int arg_num, char *arg_vet[]) {
  ...
  sc_clock clock_optical("clock_optical", GlobalParams::clock_period_optical_ps,
                         SC_PS);
  ...
  if (GlobalParams::traffic_distribution == TRAFFIC_TASKMAPPING) {
    ...
    if (GlobalParams::tgffMappingEnabled) {
      GraphParser parser;
      cout << "Loading TGFF graph file \"" << GlobalParams::taskmapping_filename
           << "\"" << endl;
      success = parser.Parse(GlobalParams::taskmapping_filename.c_str());
      // parser.PrintParsedTables(); // Add by JengDe
    } else {
      cout << "Loading task mapping configuration from file \""
           << GlobalParams::taskmapping_filename << "\"" << endl;
      success = tmInstance->createTaskGraphsFromFile(
          GlobalParams::taskmapping_filename.c_str());
    }
  ...
  if (GlobalParams::trace_mode) {
    ...
    if (GlobalParams::architecture == ARCH_HYBRID) {
      ...
      cout << "In Main.cpp, sc_trace(tf, clock_optical, \"clock_optical\");"
           << endl;
      for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
        for (int j = 0; j < GlobalParams::mesh_dim_y; j++) {
          for (int k = 0;
               k < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y; k++) {
            char label[50];

            sprintf(label, "req_opt(%02d)(%02d).to_central_router(%02d)", i, j,
                    k);
            sc_trace(tf, n->req_opt[i][j].to_central_router[k], label);
            sprintf(label, "req_opt(%02d)(%02d).from_central_router(%02d)", i,
                    j, k);
            sc_trace(tf, n->req_opt[i][j].from_central_router[k], label);

            sprintf(label, "ack_opt(%02d)(%02d).to_central_router(%02d)", i, j,
                    k);
            sc_trace(tf, n->ack_opt[i][j].to_central_router[k], label);
            sprintf(label, "ack_opt(%02d)(%02d).from_central_router(%02d)", i,
                    j, k);
            sc_trace(tf, n->ack_opt[i][j].from_central_router[k], label);

            sprintf(label, "bits_opt(%02d)(%02d).to_central_router(%02d)", i, j,
                    k);
            sc_trace(tf, n->bits_opt[i][j].to_central_router[k], label);
            sprintf(label, "bits_opt(%02d)(%02d).from_central_router(%02d)", i,
                    j, k);
  ...
  // if(GlobalParams::architecture != ARCH_MESH && GlobalParams::architecture !=
  // ARCH_HYBRID){
  ...
  cout << "Reset for " << (int)(GlobalParams::reset_time) << " cycles... "
       << endl;
  cout << "stop at " << GlobalParams::simulation_time + GlobalParams::reset_time
       << " cycles." << endl;
  srand(GlobalParams::rnd_generator_seed);
  sc_start(GlobalParams::reset_time, SC_NS);
  cout << "  Reset for " << (int)(GlobalParams::reset_time)
       << " cycles completed." << endl;
  ...
  if (tmInstance && tmInstance->taskmappinglog_file.is_open()) {
    ...
    TM_SIMSTATSLOG(endl);
    cout << "See the statistics in the file: "
         << GlobalParams::taskmappinglog_filename.c_str() << endl
         << endl;
  } else {
  ...
  // if(GlobalParams::architecture == ARCH_HYBRID)
  //   if (tmInstance) {
  //     TM_SIMWRITELOG(endl  << "Architecture is HYBRID, no WRONoC statistics."
  //     << endl);
  //   }
  ...
  if (GlobalParams::architecture != ARCH_MESH) {
    ...
    if (tmInstance && tmInstance->taskmappinglog_file.is_open()) {
      ...
      TM_SIMSTATSLOG(endl);
      cout << "See the WRONoC statistics in the file: "
           << GlobalParams::taskmappinglog_filename.c_str() << endl
           << endl;
    } else {
      cout.copyfmt(init_cout_format);
