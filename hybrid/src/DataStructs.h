...
enum FlitType {
  ...
  FLIT_TYPE_BODY,
  FLIT_TYPE_TAIL,
  FLIT_TYPE_FLOWCONTROL // Added by JengDe
};
...
struct Packet {
  ...
  void *src_task; // Added by LGGM
  void *dst_task; // Added by LGGM
  int pblock_id;  // Added by LGGM
  int path;       // added by xuanWei for path aware mapping
  ...
  void make(const int s, const int d, const double ts, const int sz) {
    ...
    src_task = NULL; // Added by LGGM
    dst_task = NULL; // Added by LGGM
    pblock_id = 0;   // Added by LGGM
    path = 0;        // added by xuanWei for path aware mapping
  }
};
...
struct Flit {
  ...
  bool flow_control; // Added by JengDe
  int path;          // Added by XuanWei

  inline bool operator==(const Flit &flit) const {
    return (flit.src_id == src_id && flit.dst_id == dst_id &&
            flit.flit_type == flit_type && flit.sequence_no == sequence_no &&
            ...
            flit.src_task == src_task && flit.dst_task == dst_task &&
            flit.pblock_id == pblock_id &&
            flit.flow_control == flow_control); // Added by JengDe
  }
};
...
struct ACK {
  ...
  inline bool operator==(const ACK &ack) const {
    return (ack.src_id == src_id && ack.dst_id == dst_id && ack.value == value);
  }
};
