...
void ProcessingElement::rxProcess() {
  if (reset.read()) {
    ...
    if (req_rx.read() == 1 - current_level_rx) {
      ...
      current_level_rx = 1 - current_level_rx; // Negate the old value for
  ...
  if (reset.read()) {
    ...
    if (ack_tx.read() == current_level_tx) {
      if (!packet_queue.empty()) {
        ...
        current_level_tx =
            1 - current_level_tx; // Negate the old value for Alternating Bit
                                  // Protocol (ABP)
...
// Added by LGGM
// *************
void ProcessingElement::peProcess() {
  if (reset.read()) {
    peGetMapping();
  } else {
    double ccycle = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;

    peMapTaskExec->timeToUnmapTasks(ccycle);
    peMapTaskExec->timeToMapTasks(ccycle);
    peMapTaskExec->timeToWakeUpTasks(ccycle);
    peMapTaskExec->timeToCheckRunningTasks(ccycle);
  }
}

void ProcessingElement::peRxProcess() {
  if (reset.read()) {
    ack_rx.write(0);
    current_level_rx = 0;
  } else {
    ...
    // cout<<"local_id = " << local_id << ", req_rx.read() = " << req_rx.read()
    // << ", current_level_rx = " << current_level_rx << endl;

    if (req_rx.read() == 1 - current_level_rx) {
      Flit flit_read = flit_rx.read();
      peMapTaskExec->processReceivedFlit(flit_read);
      cout << "In peRxProcess(), peMapTaskExec->getPeId() = "
           << peMapTaskExec->getPeId() << endl
           << endl;
      current_level_rx = 1 - current_level_rx; // Negate the old value for
                                               // Alternating Bit Protocol (ABP)

      if (flit_read.flit_type == FLIT_TYPE_TAIL) {
        TM_SIMPEEND2ENDLATENCYLOG(
            34, static_cast<tm_task *>(flit_read.src_task)->getApp()->getId(),
            static_cast<tm_task *>(flit_read.src_task)->getId(),
            static_cast<tm_task *>(flit_read.dst_task)->getId(),
            "end receiving tail_flit at " << fixed << setprecision(2)
    ...
    //     if(req_rx.read() == 1 - current_level_rx){
    //         Flit flit_read = flit_rx.read();
    //         cout << "\nIn ProcessingElement::peRxProcess(), local_id = " <<
    //         local_id << ", req_rx.read() = " << req_rx.read() << ", flit_read
    //         = " << flit_read << endl;
    //         peMapTaskExec->processReceivedFlit(flit_read);
    //         cout << "In peRxProcess(), peMapTaskExec->getPeId() = " <<
    //         peMapTaskExec->getPeId() << endl << endl; current_level_rx = 1 -
    //         current_level_rx;
    //     }
    // }
  }
...
void ProcessingElement::peTxProcess() {
  if (reset.read()) {
    req_tx.write(0);
    current_level_tx = 0;
  } else {
    ...
    if (peMapTaskExec->canShot(
            _tx_packet_queue)) { // 檢查有沒有可以傳的資料，有的話會全部放入_tx_packet_queue;
    ...
    if (ack_tx.read() == current_level_tx) {
      if (!packet_queue.empty() && packet_queue.front().size > 0) {
        ...
        Flit flit =
            peMapTaskExec->tm_nextFlit(packet_queue); // Generate a new flit
        flit_tx->write(flit);                         // Send the generated flit
        current_level_tx =
            1 - current_level_tx; // Negate the old value for Alternating Bit
                                  // Protocol (ABP)
    ...
    //     if (!packet_queue.empty() && packet_queue.front().size > 0)
    //     {
    //         Flit flit = peMapTaskExec->tm_nextFlit(packet_queue); // Generate
    //         a new flit flit_tx->write(flit); current_level_tx = 1 -
    //         current_level_tx;              // Negate the old value for
    //         Alternating Bit Protocol (ABP) req_tx.write(current_level_tx);
    //     }
    // }
  }
}

bool ProcessingElement::peGetMapping(bool reproc) {
  if (!tm_pe_ptralreadyassigned || reproc) {
    tm_pe_ptr = tmInstance->getPE(local_id);
    tm_pe_ptralreadyassigned = true;

    cout << "In peGetMapping(), before "
            "peMapTaskExec->taskStatesInitialization(tm_pe_ptr);"
         << endl;
    peMapTaskExec->taskStatesInitialization(tm_pe_ptr);
  }
  return (tm_pe_ptralreadyassigned);
}

inline bool ProcessingElement::peMappedTasks() { return (tm_pe_ptr != NULL); }
// *************
...
Flit ProcessingElement::nextFlit() {
  ...
  flit.hop_no = 0;
  flit.path = packet.path; // added by xuanWei for path aware mapping
...
bool ProcessingElement::canShot(Packet &packet) {
  ...
  } else if (GlobalParams::traffic_distribution == TRAFFIC_TASKMAPPING) {
    return false;
  } else {
    //
