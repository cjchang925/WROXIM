...
void ProcessingElement::peRxProcess() {
  if (reset.read()) {
    ...
    //cout<<"local_id = " << local_id << ", req_rx.read() = " << req_rx.read() << ", current_level_rx = " << current_level_rx << endl;

    if (req_rx.read() == 1 - current_level_rx) {
        Flit flit_read = flit_rx.read(); 
        peMapTaskExec->processReceivedFlit(flit_read);
        cout << "In peRxProcess(), peMapTaskExec->getPeId() = " << peMapTaskExec->getPeId() << endl << endl;
        current_level_rx = 1 - current_level_rx;	// Negate the old value for Alternating Bit Protocol (ABP)

        if(flit_read.flit_type == FLIT_TYPE_TAIL){
            TM_SIMPEEND2ENDLATENCYLOG(34, static_cast<tm_task*>(flit_read.src_task)->getApp()->getId(), static_cast<tm_task*>(flit_read.src_task)->getId(), static_cast<tm_task*>(flit_read.dst_task)->getId(), "end receiving tail_flit at " << fixed << setprecision(2) << (sc_time_stamp().to_double() / GlobalParams::clock_period_ps));
        }
    ...
    //     if(req_rx.read() == 1 - current_level_rx){
    //         Flit flit_read = flit_rx.read(); 
    //         cout << "\nIn ProcessingElement::peRxProcess(), local_id = " << local_id << ", req_rx.read() = " << req_rx.read() << ", flit_read = " << flit_read << endl;
    //         peMapTaskExec->processReceivedFlit(flit_read);
    //         cout << "In peRxProcess(), peMapTaskExec->getPeId() = " << peMapTaskExec->getPeId() << endl << endl;
...
void ProcessingElement::peTxProcess() {
  if (reset.read()) {
    ...
    if (peMapTaskExec->canShot(_tx_packet_queue)) { //檢查有沒有可以傳的資料，有的話會全部放入_tx_packet_queue;
    ...
    if (ack_tx.read() == current_level_tx)
    {
        if (!packet_queue.empty() && packet_queue.front().size > 0)
        {
            ...
            if(flit.flit_type == FLIT_TYPE_HEAD){
                TM_SIMPEEND2ENDLATENCYLOG(34, static_cast<tm_task*>(flit.src_task)->getApp()->getId(), static_cast<tm_task*>(flit.src_task)->getId(), static_cast<tm_task*>(flit.dst_task)->getId(), "start transmitting head_flit at " << fixed << setprecision(2) << (sc_time_stamp().to_double() / GlobalParams::clock_period_ps));
            }
        }
    }
    // }
...
bool ProcessingElement::peGetMapping(bool reproc) {
  if (!tm_pe_ptralreadyassigned || reproc) {
    ...
    cout << "In peGetMapping(), before peMapTaskExec->taskStatesInitialization(tm_pe_ptr);" << endl;
    peMapTaskExec->taskStatesInitialization(tm_pe_ptr);
  }
  return (tm_pe_ptralreadyassigned);
}

inline bool ProcessingElement::peMappedTasks() {
  return (tm_pe_ptr != NULL);
}
// *************
