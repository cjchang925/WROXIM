...
void TokenRing::updateTokenPacket(int channel) {
  if (flag[channel][token_position[channel]]->read() == RELEASE_CHANNEL) {
    ...
    LOG << "*** Token of channel " << channel << " has been assigned to hub "
        << rings_mapping[channel][token_position[channel]] << endl;

    current_token_holder[channel]->write(
        rings_mapping[channel][token_position[channel]]);
  }
}
...
void TokenRing::updateTokenMaxHold(int channel) {
  if (--token_hold_count[channel] == 0 ||
    ...
    token_hold_count[channel] =
        atoi(GlobalParams::channel_configuration[channel].macPolicy[1].c_str());
    // number of hubs of the ring
    int num_hubs = rings_mapping[channel].size();
...
void TokenRing::attachHub(int channel, int hub,
                          sc_in<int> *hub_token_holder_port,
                          sc_in<int> *hub_token_expiration_port,
                          sc_out<int> *hub_flag_port) {
  ...
  if (!current_token_holder[channel]) {
    ...
    current_token_expiration[channel]->bind(
        *(token_expiration_signals[channel]));

    // initial value that will be overwritten if mac policy != TOKEN_PACKET
    token_hold_count[channel] = 0;

    if (GlobalParams::channel_configuration[channel].macPolicy[0] !=
        TOKEN_PACKET) {
      // checking max hold cycles vs wireless transmission latency
      // consistency
      // TODO move this check: max_hold_cycles depends on the Channel not on the
      // Hub
      double delay_ps = 1000 * GlobalParams::flit_size /
                        GlobalParams::channel_configuration[channel].dataRate;
      int cycles = ceil(delay_ps / GlobalParams::clock_period_ps);
      int max_hold_cycles = atoi(
          GlobalParams::channel_configuration[channel].macPolicy[1].c_str());
      assert(cycles < max_hold_cycles);

      token_hold_count[channel] = atoi(
          GlobalParams::channel_configuration[channel].macPolicy[1].c_str());
    }
  }
  ...
  // LOG << "Attaching Hub " << hub << " to the token ring for channel " <<
  // channel << endl;
