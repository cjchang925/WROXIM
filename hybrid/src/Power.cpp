...
void Power::configureRouter(int link_width, int buffer_depth,
  ...
  assert(GlobalParams::power_configuration.routerPowerConfig.crossbar_pm.find(
             xbar_k) !=
...
void Power::configureHub(int link_width,
  ...
  // Buffer Antenna
  pair<int, int> akey =
      pair<int, int>(antenna_buffer_depth, antenna_buffer_item_size);
...
void Power::leakageBufferRouter() {
  ...
  // cout<<"In Power::leakageBufferRouter(), buffer_router_pwr_s =
  // "<<buffer_router_pwr_s<<endl;
}
...
void Power::leakageLinkRouter2Hub() {
  ...
  // cout<<"In Power::leakageLinkRouter2Hub(), link_r2h_pwr_s =
  // "<<power_static.breakdown[LINK_R2H_PWR_S].value<<endl;
}
