...
// if DEFINE_TM_SIMCONFIGLOG is defined, tm_engine writes simulation
// configuration logs to the specified log file
...
// if DEFINE_TM_SIMEXECLOG is defined, tm_engine writes simulation execution
// logs to the specified log file if TM_SIMEXECLOG_EXTENDED is defined,
// tm_engine writes extended logs
...
#define TM_SIMEXECLOG(CODE, PE, APP, TSK, MSG)                                 \
  {                                                                            \
    if (tmInstance && tmInstance->taskmappinglog_file.is_open()) {             \
      double ccycle =                                                          \
          sc_time_stamp().to_double() / GlobalParams::clock_period_ps;         \
      tmInstance->taskmappinglog_file                                          \
          << "(" << setfill('0') << setw(2) << CODE << ") "                    \
          << "@" << setfill('0') << setw(6) << ccycle << " "                   \
          << "pe" << setfill('0') << setw(3) << PE << ","                      \
          << "app" << setfill('0') << setw(2) << APP << ","                    \
          << "tsk" << setfill('0') << setw(2) << TSK << " -> " << MSG << endl; \
    }                                                                          \
  }
#else
#define TM_SIMEXECLOG(CODE, PE, APP, TSK, MSG)                         \
...
// if DEFINE_TM_SIMSTATSLOG is defined, tm_engine writes simulation statistics
...
#define TM_STRAPPTASK(T) \
  "app" << (T)->getApp()->getId() << ",task" << (T)->getId()
