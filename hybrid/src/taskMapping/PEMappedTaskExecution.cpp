...
void PEMappedTaskExecution::resetTaskStates(void) {
  ...
  nextTimeToMapTasks =
      GlobalParams::reset_time + GlobalParams::simulation_time + 1;
...
bool PEMappedTaskExecution::gotoNextPBlockExecution(void) {
  if (running_task->getCurrentPb() <
      (int)(running_task->pblocks_tasks.size() - 1)) {
    running_task->incCurrentPb();
    running_task->setExectimeLeft(
        running_task->pblocks_tasks[running_task->getCurrentPb()]
            .getExecTime());
    return true;
    ...
    if (running_task->isRootTask() &&
        running_task->getApp()->getMinRestart() == 0) {
...
bool PEMappedTaskExecution::isWaitingRxTaskReady(tm_task *t) {
  ...
  for (vector<tm_antecedent_task>::iterator it_pb_ant =
           t->antecedent_tasks.begin();
       it_pb_ant != t->antecedent_tasks.end(); ++it_pb_ant) {
    TM_ASSERT(
        it_pb_ant->getNumRx() >= 0,
        "Number of packets received without being processed is negative!");
    if (it_pb_ant->getNumRx() == 0) {
      return false;
    }
  }

  TM_SIMEXECLOG(TM_SIMEXECLOG_RECEIVESALLANT, pe->getId(), t->getApp()->getId(),
                t->getId(), "receives all antecedent payloads.");
  for (vector<tm_antecedent_task>::iterator it_pb_ant =
           t->antecedent_tasks.begin();
       it_pb_ant != t->antecedent_tasks.end(); ++it_pb_ant) {
    it_pb_ant->decNumRx(); // This task continues when all the antecedent tasks
                           // have sent their data
  }

  if (!firstTaskPBlock(
          t)) { // If the task has pblocks to execute, put it in ready task list
    return false;
  }
...
void PEMappedTaskExecution::taskToIdle() {
  ...
  if (running_task->isRootTask()) {
    TM_ASSERT(running_task->getApp()->getMinRestart() > 0,
              TM_STRAPPTASK(running_task)
                  << " with min_restart equal to zero but went to idle!");
    running_task->getApp()->setStartTime(
        ccycle + running_task->getApp()->getMinRestart());
  ...
  if (checkSleepingTasks)
    computeNextTimeToWakeUpTasks(); // Compute time to wake up tasks (maybe this
                                    // task is ready)
...
bool PEMappedTaskExecution::preparePayloadForTransmission(void) {
  ...
  if (running_task->pblocks_tasks[running_task->getCurrentPb()].getSTaskId() !=
      TM_NOT_TASK) {
    // TM_ASSERT(running_task->pblocks_tasks[running_task->getCurrentPb()].getPayLoad()
    // > 0, "PBlock with subsequent task and payload equal to zero!");
    if (running_task->pblocks_tasks[running_task->getCurrentPb()]
            .getPayLoad() == 0) {
      return false;
    }
    tx_b.setSrcTask(running_task);
    tx_b.setDstTask(tmInstance->getTask(
        running_task->getApp()->getId(),
        running_task->pblocks_tasks[running_task->getCurrentPb()]
            .getSTaskId()));
    tx_b.setPbId(running_task->getCurrentPb());
    tx_b.setPath(running_task->pblocks_tasks[running_task->getCurrentPb()]
                     .getPath()); // added by xuanWei for path aware mapping
    cout << "In preparePayloadForTransmission(), set path = " << tx_b.getPath()
         << "set pbid =" << tx_b.getPbId() << endl;
    tx_b.setStartTxCycle(
        ccycle + tmInstance->getSchedulerTxDelay()); // Starts a x cycles later
...
void PEMappedTaskExecution::computeNextTimeToMapTasks(bool reset) {
  ...
  if (pe->currentmapping != pe->mappingsonpe.end())
    nextTimeToMapTasks =
        pe->currentmapping->first - tmInstance->getSchedulerMapBefore();
  else
    nextTimeToMapTasks =
        GlobalParams::reset_time + GlobalParams::simulation_time + 1;
}

int_cycles PEMappedTaskExecution::_computeNextTimeToUnmapTask(
    tm_task *t, int_cycles _nextTimeToUnmap) {
...
void PEMappedTaskExecution::computeNextTimeToUnmapTasks(void) {
  nextTimeToUnmapTasks =
      GlobalParams::reset_time + GlobalParams::simulation_time + 1;

  // Waiting Rx Tasks
  for (list<tm_task *>::iterator it = waitingrx_tasks.begin();
       it != waitingrx_tasks.end(); ++it) {
    nextTimeToUnmapTasks =
        _computeNextTimeToUnmapTask((*it), nextTimeToUnmapTasks);
  }
  // Sleeping Tasks
  for (list<tm_task *>::iterator it = sleeping_tasks.begin();
       it != sleeping_tasks.end(); ++it) {
    nextTimeToUnmapTasks =
        _computeNextTimeToUnmapTask((*it), nextTimeToUnmapTasks);
  }
  // Ready Tasks
  for (list<tm_task *>::iterator it = ready_tasks.begin();
  ...
  if (running_task) {
    nextTimeToUnmapTasks =
        _computeNextTimeToUnmapTask((running_task), nextTimeToUnmapTasks);
  }
}
  ...
  if (sleeping_tasks.size() > 0) {
    for (list<tm_task *>::iterator it = sleeping_tasks.begin();
      ...
      if (ccycle >= (nextEvent - 1)) {
        nextTimeToWakeUpTasks =
            ccycle + 1; // Task needs to be awakened next cycle
        break;
...
void PEMappedTaskExecution::computeNextTimeToCheckRunningTasks(
    bool reset, int_cycles cycles) {
  double ccycle = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
  int_cycles nextEvent;

  if (reset) {
    nextTimeToCheckRunningTasks =
        GlobalParams::reset_time + GlobalParams::simulation_time + 1;
...
bool PEMappedTaskExecution::switchTaskContext() {
  ...
  if (!ready_tasks.empty()) {
    ...
    schedulerTicksLeft =
        tmInstance->getSchedulerDelay() + tmInstance->getSchedulerTick();
    computeNextTimeToCheckRunningTasks(false, tmInstance->getSchedulerDelay());

    // Statistics - PE Idle Cycles - It must be also done when tasks are
    // unmapped
    double ccycle = sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
...
void PEMappedTaskExecution::tasksInRunningState() {
  ...
  TM_ASSERT(running_task != NULL,
            "tasksInRunningState function called without a task in execution!");

  // Execution: pblock, context switching, transmission
  int_cycles cyclesSinceLastEvent = (ccycle - lastTimeRunningTasksWereChecked);
  TM_ASSERT(cyclesSinceLastEvent <= schedulerTicksLeft,
            "Context Switching was performed in a time greater than the "
            "Scheduler Ticks Left!");
  schedulerTicksLeft -= cyclesSinceLastEvent;

  if (running_task->isInContextSwitchingState()) {
    running_task->toRunningState();
    TM_ASSERT(cyclesSinceLastEvent == tmInstance->getSchedulerDelay(),
              "Context Switching took longer than expected!");
    TM_ASSERT(
        schedulerTicksLeft >= TM_PE_SCHEDULER_TICK_MIN,
        "Scheduler Tick is less than " << TM_PE_SCHEDULER_TICK_MIN << "!");
#ifdef DEFINE_TM_SIMEXECLOG
    if (running_task->getExectimeLeft() ==
        running_task->pblocks_tasks[running_task->getCurrentPb()]
            .getExecTime()) {
      TM_SIMEXECLOG(TM_SIMEXECLOG_STARTSEXEC, pe->getId(),
                    running_task->getApp()->getId(), running_task->getId(),
                    "starts its execution with pb"
                        << running_task->getCurrentPb() << ".");
    } else {
      TM_SIMEXECLOG(TM_SIMEXECLOG_CONTINUESEXECAFTERCONEXTSW, pe->getId(),
                    running_task->getApp()->getId(), running_task->getId(),
                    "continues its execution with pb"
                        << running_task->getCurrentPb() << ".");
    }
#endif
    ...
    TM_ASSERT(running_task->isInRunningState(),
              "Task is not in running state!");
    running_task->setExectimeLeft(running_task->getExectimeLeft() -
                                  cyclesSinceLastEvent);
    TM_ASSERT(running_task->getExectimeLeft() >= 0,
              "Execution Time Left must be greater than or equal to zero: app"
                  << running_task->getApp()->getId() << ", task"
                  << running_task->getId() << "!");
  ...
  while (running_task->getExectimeLeft() == 0) {
    TM_SIMEXECLOG(
        TM_SIMEXECLOG_COMPLETESPB, pe->getId(), running_task->getApp()->getId(),
        running_task->getId(),
        "pb" << running_task->getCurrentPb() << " completes its processing.");

    if (preparePayloadForTransmission()) {
      TM_SIMEXECLOG(TM_SIMEXECLOG_PAYLOADREADYTOTRANSF, pe->getId(),
                    running_task->getApp()->getId(), running_task->getId(),
                    "payload of pb" << running_task->getCurrentPb()
                                    << " is ready to be transferred.");
    }

    if (!gotoNextPBlockExecution()) {
      TM_SIMEXECLOG(TM_SIMEXECLOG_COMPLETESALLPBS, pe->getId(),
                    running_task->getApp()->getId(), running_task->getId(),
      ...
      TM_SIMEXECLOG(
          TM_SIMEXECLOG_CONTINUESEXEC, pe->getId(),
          running_task->getApp()->getId(), running_task->getId(),
...
void PEMappedTaskExecution::tasksToBeMapped() {
  ...
  for (vector<tm_task *>::iterator it =
           pe->currentmapping->second.tasks_on_pe.begin();
       it != pe->currentmapping->second.tasks_on_pe.end(); ++it) {
    t = (*it);

    t->toBeingMappedState(pe->getId(), pe->currentmapping->first,
                          pe->currentmapping->second.getStopTime());
...
void PEMappedTaskExecution::tasksToBeUnmapped() {
  ...
  for (list<tm_task *>::iterator it = waitingrx_tasks.begin();
    ...
    if (t->getApp()->alreadyStopped()) {
      ...
      it = waitingrx_tasks.erase(
          it); // Just remove the task from the waiting rx list
  ...
  for (list<tm_task *>::iterator it = sleeping_tasks.begin();
    ...
    if (t->getApp()->alreadyStopped()) {
      t->toStopAndUnmapState();
      it = sleeping_tasks.erase(
          it); // Just remove the task from the sleeping list
  ...
  if (running_task) {
    if (running_task->getApp()->alreadyStopped()) {
      // Statistics (tasksInRunningState won't collect it because the task is
      // being stopped)
      double ccycle =
          sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
      int_cycles cyclesSinceLastEvent =
          (ccycle - lastTimeRunningTasksWereChecked);
      if (running_task->isInContextSwitchingState()) {
        pe->stats.statAddCSwitchingCycles(cyclesSinceLastEvent);
      } else {
        pe->stats.statAddTskExecCycles(cyclesSinceLastEvent);
  ...
  TM_ASSERT(waitingRxTaskStopped || sleepingTaskStopped || readyTaskStopped ||
                runningTaskStopped,
  ...
  if (lastTimePeWentIdle != -1) {
    if (ready_tasks.size() == 0 && sleeping_tasks.size() == 0 &&
        waitingrx_tasks.size() == 0 && running_task == NULL) {
      double ccycle =
          sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
      pe->stats.statAddIdleCycles(ccycle - lastTimePeWentIdle);
      lastTimePeWentIdle = -1;
  ...
  TM_SIMPEMETRICSLOG(TM_SIMSTATSLOG_STATPECYCLESANDENERGY, pe->getId(),
                     "acum_cycles_performed",
                     "contextswitching="
                         << pe->stats.statGetCSwitchingCycles() << ","
...
void PEMappedTaskExecution::taskStatesInitialization(tm_pe *pe) {
  ...
  if (pe) {
    ...
    TM_ASSERT(pe->mappingsonpe.size() > 0,
              "PE" << pe->getId() << " does not have mappings!");
...
void PEMappedTaskExecution::timeToUnmapTasks(double ccycle) {
  TM_ASSERT(ccycle <= nextTimeToUnmapTasks,
            "Time to unmap tasks already passed!");
  if (ccycle == nextTimeToUnmapTasks)
    tasksToBeUnmapped();
}

void PEMappedTaskExecution::timeToWakeUpTasks(double ccycle) {
  TM_ASSERT(ccycle <= nextTimeToWakeUpTasks,
            "Time to wake up tasks already passed!");
  if (ccycle == nextTimeToWakeUpTasks)
    tasksInSleepingState();
}

void PEMappedTaskExecution::timeToCheckRunningTasks(double ccycle) {
  TM_ASSERT(ccycle <= nextTimeToCheckRunningTasks,
            "Time to check running tasks already passed!");
  ...
  while (!waitingpb_tx.empty()) {
    ...
    if (ccycle >= tx_b->getStartTxCycle()) {
      ...
      if (p.src_id == p.dst_id) {
        TM_SIMEXECLOG(TM_SIMEXECLOG_PLBEINGTRANSTOSAMEPE, t_src->getPeId(),
                      t_src->getApp()->getId(), t_src->getId(),
                      "payload of pb"
                          << p.pblock_id
                          << " being transferred to task in the same pe.");
      } else if (GlobalParams::architecture == ARCH_MESH) {
        TM_SIMEXECLOG(
            TM_SIMEXECLOG_PLBEINGTRANSTOWNOC, t_src->getPeId(),
            t_src->getApp()->getId(), t_src->getId(),
            "payload of pb" << p.pblock_id << " being transferred to WNoC.");
      } else {
        TM_SIMEXECLOG(TM_SIMEXECLOG_PLBEINGTRANSTOWNOC, t_src->getPeId(),
                      t_src->getApp()->getId(), t_src->getId(),
...
Flit PEMappedTaskExecution::_tm_nextFlit(Packet &p) {
  ...
  cout << "In _tm_nextFlit(), set path = " << flit.path
       << "set pbid =" << flit.pblock_id << endl;
  if (p.size == p.flit_left) {
    ...
    if (flit.src_id != flit.dst_id) {
      t_src->getApp()
          ->app_mappings[appCurrentMapping]
          .stat_txstart_delay.statAddNewValue(ccycle - p.timestamp);
    }
...
void PEMappedTaskExecution::processReceivedFlit(Flit flit) {
  ...
  TM_ASSERT(
      flit.src_task && flit.dst_task,
      "Flit received is wrong. No source and destination Tasks received!");

  t_src = (tm_task *)flit.src_task;
  t_dst = (tm_task *)flit.dst_task;
  cout << "In processReceivedFlit(), t_src->getId() = " << t_src->getId()
       << endl;
  cout << "In processReceivedFlit(), t_dst->getId() = " << t_dst->getId()
       << endl;
  ...
  TM_ASSERT(t_dst->getApp()->getCurrentMappingInt() >= 0,
            "Current Mapping is -1 when task " << t_dst->getId()
                                               << " received data!");
  ...
  if (flit.flit_type == FLIT_TYPE_TAIL) {
    if (!t_dst->getApp()->alreadyStopped()) {
      ...
      for (vector<tm_antecedent_task>::iterator it_at =
               t_dst->antecedent_tasks.begin();
           it_at != t_dst->antecedent_tasks.end(); it_at++) {
        t_at = &(*it_at);
        if (t_at->getATask() == t_src && t_at->getPbId() == src_pblock_id) {
...
#ifdef DEFINE_TM_SIMEXECLOG
          if (flit.src_id == flit.dst_id) {
            TM_SIMEXECLOG(TM_SIMEXECLOG_RECEIVESPLFROMSAMEPE, pe->getId(),
                          t_dst->getApp()->getId(), t_dst->getId(),
                          "receives payload of pb" << src_pblock_id << " (task"
                                                   << t_src->getId()
                                                   << ") from same PE.");
          } else {
            TM_SIMEXECLOG(TM_SIMEXECLOG_RECEIVESPLFROMWNOC, pe->getId(),
                          t_dst->getApp()->getId(), t_dst->getId(),
                          "receives payload of pb" << src_pblock_id << " (task"
                                                   << t_src->getId()
                                                   << ") from WNoC.");
          ...
          if (t_dst->isInWaitingRxState()) {
            cout << "In processReceivedFlit(), t_dst isInWaitingRxState, "
                 << "t_dst->getPeId() = " << t_dst->getPeId() << endl;
            cout << "In processReceivedFlit(), waitingrx_tasks.size() = "
                 << waitingrx_tasks.size() << endl;
          ...
          if (flit.src_id != flit.dst_id) {
            TM_SIMEND2ENDLATENCYLOG(
                TM_SIMSTATSLOG_STATEND2ENDLATENCY, t_dst->getApp()->getId(),
                t_src->getId(), t_dst->getId(),
            ...
            TM_SIMEND2ENDHOPSLOG(
                TM_SIMSTATSLOG_STATEND2ENDHOPCOUNT, t_dst->getApp()->getId(),
                t_src->getId(), t_dst->getId(),
            ...
            // TOTAL Statistics
            int_cycles start;
            TM_ASSERT((start = t_st->_stat_headflitsent) > 0,
                      "Unexpected behavior in the Flits!");
            t_st->total_stat_commtime[appCurrentMapping].statAddNewValue(
                ccycle - start);
            ...
            t_src->getApp()
                ->app_mappings[appCurrentMapping]
                .stat_end2end_latency.statAddNewValue(t_st->stat_flit_latency);
            t_src->getApp()
                ->app_mappings[appCurrentMapping]
                .stat_hopcount.statAddNewValue(t_st->stat_flit_hops);
          }
          break;
        }
      } // End for
      if (!dst_pblock_found) {
        TM_ASSERT(false, "There is a packet received at PE "
                             << t_dst->getPeId()
                             << " that was not taken by any pb!");
      }
    }
    // Task already stopped
    else {
      t_src->getApp()->app_mappings[appCurrentMapping].stat_flits_ignored +=
          flit.sequence_length;
      TM_SIMEXECLOG(TM_SIMEXECLOG_DESTNOTREACHEABLE, pe->getId(),
                    t_dst->getApp()->getId(), t_dst->getId(),
...
int PEMappedTaskExecution::getPeId() { return pe->getId(); }
