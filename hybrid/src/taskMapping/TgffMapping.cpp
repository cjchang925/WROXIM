...
bool GraphParser::Parse(const char *fname) {
  ...
  if (infile.is_open()) {
    ...
    while (infile) {
      ...
      } else if (keyWord.compare("@COMMUN") == 0 ||
                 state != FSM_PARSER_DATA_TABLE_IDLE) {
...
void GraphParser::ParseDataBlock(string line) {
  ...
  // DEBUG
  // cout << typeId << " " << execTimeTable[typeId] << " " <<
  // commVolumTable[typeId] << " " << endl;
}

void GraphParser::BuildTaskGraphMapping(int appId, int_cycles start,
                                        int_cycles end) {
  // Todo: if there is only one leaf task, this can be simplifed.
  // First Set execute time for each task
  for (map<int, int>::iterator it = taskTable.begin(); it != taskTable.end();
       ++it) {
    int taskId = it->first;
    int type = it->second;

    TM_ASSERT(execTimeTable.count(taskId) > 0,
              " Cannot find src task in execute time table.");

    tmInstance->addPBlockToTask(appId, taskId, execTimeTable[type], TM_NOT_TASK,
                                0, 0);
  }

  // Then add PB for each task
  for (map<pair<int, int>, int>::iterator it = arcTable.begin();
       it != arcTable.end(); ++it) {
    ...
    TM_ASSERT(execTimeTable.count(type) > 0,
              " Cannot find execute time type .");
    TM_ASSERT(commVolumTable.count(type) > 0,
              " Cannot find communication volume type .");

    tm_task *srcTask = &tmInstance->tm_apps[appId].task_graph[src];
    if (srcTask->pblocks_tasks.size() == 1 &&
        srcTask->pblocks_tasks[0].getSTaskId() == TM_NOT_TASK) {
      // Overriding first PB
...
void GraphParser::PrintParsedTables(void) {
  ...
  for (map<pair<int, int>, int>::iterator it = arcTable.begin();
    ...
    cout << "Src Task: " << key.first << " Dst Task: " << key.second
         << " Type:" << it->second << endl;
  }
  cout << endl;
  cout << "************************" << endl;
  ...
  cout << "************************" << endl;
  cout << endl;
  for (map<int, int>::iterator it = execTimeTable.begin();
       it != execTimeTable.end(); ++it) {
  ...
  cout << "************************" << endl;
  cout << endl;
  for (map<int, int>::iterator it = commVolumTable.begin();
       it != commVolumTable.end(); ++it) {
