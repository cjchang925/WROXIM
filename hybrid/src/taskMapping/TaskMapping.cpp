...
void tm_appmapping::statLeafExecFinished(tm_task *t_leaf)
{
	...
	stat_commtime = t_leaf->getTotalLatencyUptoThis(t_leaf->getApp()->getCurrentMappingInt());
}
...
void tm_app::printSubsequentCommTimePerBranch(int imap, tm_task *t, string initss, tm_appexec_statistics<int> initstat)
{
	...
	for (unsigned int ind_pb = 0; ind_pb < t->pblocks_tasks.size(); ++ind_pb)
	{
		...
		ssfromthis << t->getId() << "-" << t_st->getId() << "="
				   << t->pblocks_tasks[ind_pb].total_stat_commtime[imap].statGetExecNumber() << ","
				   << t->pblocks_tasks[ind_pb].total_stat_commtime[imap].statGetMin() << ","
				   << (int)t->pblocks_tasks[ind_pb].total_stat_commtime[imap].statGetAverage() << ","
		...
		{
			ssfromthis << " -> ";
			printSubsequentCommTimePerBranch(imap, t_st, initss + ssfromthis.str(), initstat + statfromthis);
		}
		else
		{
			tm_appexec_statistics<int> totalstat = initstat + statfromthis;
			ssfromthis << " -->> T(min,avg,max)="
					   // << totalstat.statGetExecNumber() << ","  // real payloads transmitted in the Branch from root to leaf
					   << totalstat.statGetMin() << ","
					   << (int)totalstat.statGetAverage() << ","
					   << totalstat.statGetMax();
			TM_SIMSTATSLOG("  br" << currentBranch << ": " << initss << ssfromthis.str() << " (cycles)" << endl);
			currentBranch++;
		}
	}
}
						...
						int &stat_flits_to_samepe)
{
	for (unsigned int imap = 0; imap < app_mappings.size(); imap++)
	{
		...
		TM_SIMSTATSLOG("*** App" << _id << " - Mapping" << imap << " ***" << endl
								 << "Execution Time (CPU + Comm.) (executions,min,avg,max)=" << app_mappings[imap].stat_exectime.statGetExecNumber() << ","
								 << app_mappings[imap].stat_exectime.statGetMin() << ","
								 << (int)app_mappings[imap].stat_exectime.statGetAverage() << ","
								 << app_mappings[imap].stat_exectime.statGetMax() << " (cycles)" << endl);
		TM_SIMSTATSLOG("Communication Time per Branch (payloads,min,avg,max)" << endl);
		...
		TM_SIMSTATSLOG("Communication Time Critical Branch (min,avg,max)="
					   // << app_mappings[imap].stat_commtime.statGetExecNumber() << "," // real payloads transmitted in the Branch from root to leaf
		...
		TM_SIMSTATSLOG("FLITs Ignored due to Application Unmapping="
					   << app_mappings[imap].stat_flits_ignored
					   << endl);
		stat_end2end_latency.statAddNewValue(app_mappings[imap].stat_end2end_latency);
		TM_SIMSTATSLOG("FLIT End2End Latency (flits,min,avg,max)=" << app_mappings[imap].stat_end2end_latency.statGetExecNumber() << ","
																   << app_mappings[imap].stat_end2end_latency.statGetMin() << ","
																   << app_mappings[imap].stat_end2end_latency.statGetAverage() << ","
																   << app_mappings[imap].stat_end2end_latency.statGetMax() << " (cycles)" << endl);
		stat_hopcount.statAddNewValue(app_mappings[imap].stat_hopcount);
		TM_SIMSTATSLOG("FLIT End2End HopCount (flits,min,avg,max)=" << app_mappings[imap].stat_hopcount.statGetExecNumber() << ","
																	<< app_mappings[imap].stat_hopcount.statGetMin() << ","
																	<< app_mappings[imap].stat_hopcount.statGetAverage() << ","
																	<< app_mappings[imap].stat_hopcount.statGetMax() << endl);
		TM_SIMSTATSLOG("FLIT Communication Cost (flits x average hopcount)="
					   << app_mappings[imap].stat_hopcount.statGetExecNumber() * app_mappings[imap].stat_hopcount.statGetAverage()
					   << endl);
		TM_SIMSTATSLOG(endl);
	}
}
...
void tm_task::toStopAndUnmapState()
{
	...
	if (isRootTask())
	{
		TM_SIMEXECLOG(TM_SIMEXECLOG_STOPSANDUNMAPPED, _peid_container, _ptr_parentapp->getId(), _id, "stops and is unmapped at @" << ccycle << " (expected at @" << _ptr_parentapp->getStopTime() << ").");
		_ptr_parentapp->setStopTime(ccycle);
	}
	else
	{
		TM_SIMEXECLOG(TM_SIMEXECLOG_STOPSANDUNMAPPED, _peid_container, _ptr_parentapp->getId(), _id, "stops and is unmapped at @" << ccycle << " (root stopped at @" << _ptr_parentapp->getStopTime() << ").");
	}
...
void tm_task::taskExecutionCompleted()
{
	...
	else if (isLeafTask())
	{
		...
		TM_SIMAPPMETRICSLOG(TM_SIMSTATSLOG_STATAPPEXECTIME,
							_ptr_parentapp->getId(), "exec_finished",
							"exectime=" << _ptr_parentapp->getCurrentStatsExecTime()->statGetLast());
	}
}
...
tm_appexec_statistics<int> tm_task::getTotalLatencyUptoThis(int imap)
{
	...
	for (vector<tm_antecedent_task>::iterator it_at = antecedent_tasks.begin(); it_at != antecedent_tasks.end(); it_at++)
	{
		...
		_stat_branch = t_st->total_stat_commtime[imap] + (*it_at).getATask()->getTotalLatencyUptoThis(imap);
...
void TaskMapping::addPBlockToTask(int appid, int taskid, int_cycles exectime, int staskid, int payload, int path)
{
	tm_app *app;
	tm_task *t;
	tm_pblock_task p;
	...
	else if (staskid != TM_NOT_TASK)
	{
		...
		TM_WARNING("PBlock " << (t->pblocks_tasks.size() + 1) << " of task " << taskid << " in app" << appid << " has a payload equal to zero. Subsequent task is ignored!");
	}
...
bool TaskMapping::checkConsistency(void)
{
	...
	for (map<int, tm_app>::iterator it_app = tm_apps.begin(); it_app != tm_apps.end(); ++it_app)
	{
		...
		{ // Check if the start time of each mapping meets the requirements
...
void TaskMapping::showAppStatistics(void)
{
	...
	TM_SIMSTATSLOG("Execution Time (CPU + Comm.) (executions,min,avg,max)=" << stat_exectime.statGetExecNumber() << ","
	...
	TM_SIMSTATSLOG("FLITs Ignored due to Application Unmapping="
				   << stat_flits_ignored
				   << endl);
	TM_SIMSTATSLOG("FLIT End2End Latency (flits,min,avg,max)=" << stat_end2end_latency.statGetExecNumber() << ","
															   << stat_end2end_latency.statGetMin() << ","
															   << stat_end2end_latency.statGetAverage() << ","
															   << stat_end2end_latency.statGetMax() << " (cycles)" << endl);
	TM_SIMSTATSLOG("FLIT End2End HopCount (flits,min,avg,max)=" << stat_hopcount.statGetExecNumber() << ","
																<< stat_hopcount.statGetMin() << ","
																<< stat_hopcount.statGetAverage() << ","
																<< stat_hopcount.statGetMax() << endl);
	TM_SIMSTATSLOG("FLIT Communication Cost (flits x average hopcount)="
				   << stat_hopcount.statGetExecNumber() * stat_hopcount.statGetAverage()
				   << endl);
		...
		while (infile)
		{
			...
			if (line.length() >= strapp.length() && line.compare(0, strapp.length(), strapp) == 0)
			{
				...
				while (segline)
				{
					...
					cout << "In TaskMapping::createTaskGraphsFromFile(), segline = " << seg << endl;
				...
				while (segline)
				{
					...
					case FSMTASK_PBLOCK_ST:
						...
						if (valueRead == TM_NOT_TASK)
						{
							...
							cout << "Set Last Task ID of App " << appid << " to " << id << endl;
						}
					...
					case FSMTASK_PATH:
						...
						{
							path = valueRead;
							// TM_ASSERT(path >= TM_PATH_MIN, "Path (" << path << ") cannot be less than " << TM_PATH_MIN << "!");
						}
