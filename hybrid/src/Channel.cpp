#include "Channel.h"
void Channel::b_transport(int id, tlm::tlm_generic_payload &trans,
                          sc_time &delay) {
...
void Channel::accountWirelessRxPower() {
  for (unsigned int i = 0; i < hubs.size(); i++) {
    ...
    else if (!(hubs[i]->power.isSleeping()))
      hubs[i]->power.wirelessDynamicRx();
  }
}
...
void Channel::powerManager(int hub_dst_index, tlm::tlm_generic_payload &trans) {
  ...
  if (f->flit_type == FLIT_TYPE_HEAD) {
    ...
    for (unsigned int i = 0; i < hubs.size(); i++) {
      ...
      if (i != (unsigned int)hub_dst_index) {
        hubs[i]->power.rxSleep(sleep_cycles);
        LOG << " HUB_" << hubs_id[i] << " rxSleep() invoked with "
            << sleep_cycles << " cycles " << endl;
      }
    }
  }
}

bool Channel::get_direct_mem_ptr(int id, tlm::tlm_generic_payload &trans,
...
void Channel::invalidate_direct_mem_ptr(int id, sc_dt::uint64 start_range,
