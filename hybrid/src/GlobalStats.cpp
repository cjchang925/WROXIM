  ...
  for (int y = 0; y < GlobalParams::mesh_dim_y; y++)
    for (int x = 0; x < GlobalParams::mesh_dim_x; x++) {
      if (GlobalParams::architecture == ARCH_MESH) {
        ...
        if (received_packets) {
          avg_delay +=
              received_packets * noc->t[x][y]->r->stats.getAverageDelay();
        ...
        if (received_packets) {
          avg_delay +=
              received_packets * noc->t_ONoC[x][y]->r->stats.getAverageDelay();
          total_packets += received_packets;
        }
      }
    }
...
void GlobalStats::showStats(std::ostream &out, bool detailed) {
  if (detailed) {
    ...
    for (int y = 0; y < GlobalParams::mesh_dim_y; y++)
      for (int x = 0; x < GlobalParams::mesh_dim_x; x++)
        noc->t[x][y]->r->stats.showStats(y * GlobalParams::mesh_dim_x + x, out,
                                         true);
    out << "];" << endl;
...
void GlobalStats::showTop10DynamicPower(std::ostream &out) {
  ...
  for (int i = 0; i < GlobalParams::mesh_dim_y; ++i) {
    for (int j = 0; j < GlobalParams::mesh_dim_x; ++j) {
      ...
      if (GlobalParams::architecture == ARCH_HYBRID) {
        if (!noc->t_Hybrid[i][j] || !noc->t_Hybrid[i][j]->r)
          continue;
        breakdown_ptr =
            noc->t_Hybrid[i][j]->r->power.getDynamicPowerBreakDown();
      } else if (GlobalParams::architecture == ARCH_MESH) {
        if (!noc->t[i][j] || !noc->t[i][j]->r)
          continue;
        breakdown_ptr = noc->t[i][j]->r->power.getDynamicPowerBreakDown();
      } else {
        if (!noc->t_ONoC[i][j] || !noc->t_ONoC[i][j]->r)
          continue;
  ...
  for (int i = 0; i < 10 && i < (int)breakdown.size(); ++i) {
    out << "% " << i + 1 << ". " << breakdown[i].second << ": "
        << breakdown[i].first << " J" << std::endl;
  }
}
  ...
  for (int i = 0; i < GlobalParams::mesh_dim_y; ++i) {
    for (int j = 0; j < GlobalParams::mesh_dim_x; ++j) {
      ...
      if (!breakdown_ptr)
        continue;
      for (int k = 0; k < NO_BREAKDOWN_ENTRIES_D; ++k) {
        cout << "breakdown_ptr->breakdown[" << k
             << "].label: " << breakdown_ptr->breakdown[k].label
             << ", value: " << breakdown_ptr->breakdown[k].value << endl;
        total[k] += breakdown_ptr->breakdown[k].value;
        labels[k] = breakdown_ptr->breakdown[k].label;
      }
    }
...
void GlobalStats::showWRONoCStats(std::ostream &out) {
  ...
  out << "% \t Laser energy (J): " << getLaserEnergy() + getLaserLossEnergy()
      << endl;
  ...
  // out << "% \t\t Arbitration and Flow Control energy (J): " <<
  // getArbitrationFlowControlEnergy() << endl;
}
