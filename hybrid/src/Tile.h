...
SC_MODULE(Tile) {
  ...
  // sc_in <bool> hub_req_rx;	        // The requests associated with the
  // input channels sc_out <bool> hub_ack_rx;	        // The outgoing ack
  // signals associated with the input channels

  // sc_out <Flit> hub_flit_tx;	// The output channels
  // sc_out <bool> hub_req_tx;	        // The requests associated with the
  // output channels sc_in <bool> hub_ack_tx;	        // The outgoing ack
  // signals associated with the output channels
  ...
  sc_signal<bool>
      hub_req_tx; // The requests associated with the output channels
...
//! Central Router (without PE, receives Flits from all tiles and transmits them
//! to all tiles)
SC_MODULE(Tile_Central) {
  ...
  void transfer_flit() {
    ...
    for (int i = 0; i < num_ports; i++) {
      for (int j = 0; j < num_ports; j++) {
        bool send = false;
        if ((flit_rx[i][j].event() || req_rx[i][j].event() ||
             bits_in[i][j].event())) {
          send = true;
          ...
          if (dst_id == 0 && src_id == 0) {
            ...
            if (flit_rx[i][j].event())
              cout << "flit_rx[" << i << "][" << j
                   << "].event() = true, flit_rx[" << i << "][" << j
                   << "].read() = " << flit_rx[i][j].read() << endl;
            else
              cout << "flit_rx[" << i << "][" << j
                   << "].event() = false, flit_rx[" << i << "][" << j
                   << "].read() = " << flit_rx[i][j].read() << endl;
            if (req_rx[i][j].event())
              cout << "req_rx[" << i << "][" << j << "].event() = true, req_rx["
                   << i << "][" << j << "].read() = " << req_rx[i][j].read()
                   << endl;
            else
              cout << "req_rx[" << i << "][" << j
                   << "].event() = false, req_rx[" << i << "][" << j
                   << "].read() = " << req_rx[i][j].read() << endl;
            if (bits_in[i][j].event())
              cout << "bits_in[" << i << "][" << j
                   << "].event() = true, bits_in[" << i << "][" << j
                   << "].read() = " << bits_in[i][j].read() << endl;
            else
              cout << "bits_in[" << i << "][" << j
                   << "].event() = false, bits_in[" << i << "][" << j
                   << "].read() = " << bits_in[i][j].read() << endl;
            cout << "ack_tx[" << dst_id << "][" << src_id << "].write("
                 << ack_rx[i][j].read() << ");" << endl;
        ...
        if ((ack_rx[i][j].event() || bits_in[i][j].event()) && !send) {
          ...
          cout << "In Tile_Central, transfer_ack: src_id -> dst_id = " << src_id
               << " -> " << dst_id << endl
               << endl;
          if (dst_id >= 0 && dst_id < num_ports) {
            ...
            SC_REPORT_ERROR("Tile_Central",
                            "Invalid destination ID by ack_rx[i][j].event()");
          }
        }
      }
    }
...
SC_MODULE(Tile_ONoC) {
  ...
  sc_in<bool> *req_rx_from_central_tile;  // The requests associated with the
                                          // input channels
  sc_in<sc_logic> *bits_in_from_central_tile;
  sc_out<ACK> *ack_rx_to_central_tile; // The outgoing ack signals associated
                                       // with the input channels

  sc_out<Flit> *flit_tx_to_central_tile; // The output channels
  sc_out<bool> *req_tx_to_central_tile;  // The requests associated with the
                                         // output channels
  sc_out<sc_logic> *bits_out_to_central_tile;
  sc_in<ACK> *ack_tx_from_central_tile; // The outgoing ack signals associated
  ...
  Tile_ONoC(sc_module_name nm, int id) : sc_module(nm) {
    ...
    // cout << "Tile_ONoC's all signals and ports have been initialized." <<
    // endl;
    ...
    // cout << "Tile_ONoC's all serializers and deserializers have been
    // initialized." << endl;
    ...
    pe->free_slots_neighbor(
        free_slots_neighbor_local); //* not used in WRONoC //dummy_signal
  }
  ~Tile_ONoC() {
...
SC_MODULE(Tile_Hybrid) {
  ...
  // sc_in <bool> hub_req_rx_2onoc;	        // The requests associated with
  // the input channels sc_out <bool> hub_ack_rx_2onoc;	        // The outgoing
  // ack signals associated with the input channels

  // sc_out <Flit> hub_flit_tx_2onoc;	// The output channels
  // sc_out <bool> hub_req_tx_2onoc;	        // The requests associated with
  // the output channels sc_in <bool> hub_ack_tx_2onoc;	        // The outgoing
  // ack signals associated with the output channels
