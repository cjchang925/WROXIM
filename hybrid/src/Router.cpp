  ...
  if (reset.read()) {
    ...
    for (int i = 0; i < DIRECTIONS + 2; i++) {
      ...
      if ((req_rx[i].read() == 1 - current_level_rx[i]) &&
        ...
        cout << "In Router_ENoC::rxProcess_PE(), received_flit.src_id = "
             << received_flit.src_id
             << ", req_rx[0].read() = " << req_rx[0].read() << endl;
        // *************
        // Added by LGGM
        // *************
        ...
        // cout<< "current_level_rx[" << i << "] = " << current_level_rx[i] <<"
        // received_flit = "<< received_flit << endl;
  ...
  if (reset.read()) {
    ...
    for (int i = 0; i < DIRECTIONS + 2; i++) {
      if (!buffer[i].IsEmpty()) {
        ...
        cout << "router" << local_id << " flit = " << flit << "  o =" << o
             << endl;
        // if(i == DIRECTION_HUB)
        // cout<<o<<endl;

        if (o != NOT_RESERVED) {
          // cout << "current_level_tx[o] = " << current_level_tx[o] << ",
          // ack_tx[o].read() = " << ack_tx[o].read() << endl;
          if (current_level_tx[o] == ack_tx[o].read()) {
            // if (GlobalParams::verbose_mode > VERBOSE_OFF)
            LOG << "Input[" << i << "] forward to Output[" << o
                << "], flit: " << flit << endl;
            ...
            //
            if ((flit.flit_type == FLIT_TYPE_TAIL && rtable_cnt[o] == 0) &&
                (o == 4 && GlobalParams::architecture == ARCH_HYBRID))
              // if ((flit.flit_type == FLIT_TYPE_TAIL ))
              reservation_table.release(o);

            // if (((flit.flit_type == FLIT_TYPE_TAIL )&& o != 4) ||
            // GlobalParams::architecture!= ARCH_HYBRID)
            if ((flit.flit_type == FLIT_TYPE_TAIL) &&
                (o != 4 || GlobalParams::architecture != ARCH_HYBRID))
              reservation_table.release(o);
            ...
            LOG << " cannot forward Input[" << i << "] forward to Output[" << o
                << "], flit: " << flit << endl;
            if (flit.flit_type == FLIT_TYPE_HEAD) {
              rtable_cnt[o]--;
              cout << "router_id = " << local_id << " rtable_cnt[" << o
...
void Router::perCycleUpdate() {
  if (reset.read()) {
    ...
    // cout<< "In Router::perCycleUpdate(), power leakage calculation starts."
    // << endl;
    power.leakageRouter();
    for (int i = 0; i < DIRECTIONS + 1; i++) {
...
vector<int> Router::routingFunction(const RouteData &route_data) {
  if (GlobalParams::use_winoc) {
    if (hasRadioHub(local_id) && hasRadioHub(route_data.dst_id) &&
        !sameRadioHub(local_id, route_data.dst_id)) {
      LOG << "Setting direction HUB to reach destination node "
...
int Router::route(const RouteData &route_data) {
  ...
  if (GlobalParams::architecture == ARCH_MESH) {
    ...
    // cout << "In Router::route(), route_data.path = " << route_data.path <<
    // endl;
...
unsigned int Router::getFlitsCount() {
  ...
  for (int i = 0; i < DIRECTIONS + 2; i++) count += buffer[i].Size();
...
void Router::ShowBuffersStats(std::ostream &out) {
  for (int i = 0; i < DIRECTIONS + 2; i++) buffer[i].ShowStats(out);
}
...
void Router_ONoC::rxProcess_PE() {
  if (reset.read()) {
    ...
    if ((req_rx[0].read() == 1 - current_level_rx[0]) &&
        (tx_buffer[received_flit.dst_id].Size() <
         tx_buffer[received_flit.dst_id].GetMaxBufferSize() - 1)) {

      cout << "In Router_ONoC::rxProcess_PE(), received_flit.src_id = "
           << received_flit.src_id
           << ", req_rx[0].read() = " << req_rx[0].read() << endl;
      ...
      if (received_flit.dst_id == local_id) {
        string t =
            "\nIn Router_ONoC::rxProcess_PE(), Error condition -> "
            "received_flit.dst_id == local_id.";
        TM_ASSERT(false, t);
        assert(false);
      } else {
        tx_buffer[received_flit.dst_id].Push(received_flit);
        // cout << "In Router_ONoC::rxProcess_PE(), local_id = "<<local_id<<",
        // Push src_task =
        // "<<static_cast<tm_task*>(received_flit.src_task)->getId()<<",
        // dst_task =
        // "<<static_cast<tm_task*>(received_flit.dst_task)->getId()<<", flit :"
        // << received_flit << " into tx_buffer" << endl;
        tx_buffer[received_flit.dst_id].Print("tx_buffer");
        // cout << "In Router_ONoC::rxProcess_PE(), local_id = "<<local_id<<",
        // tx_buffer[" << received_flit.dst_id << "].Size() = " <<
        // tx_buffer[received_flit.dst_id].Size() << endl;
      }
      ...
      // cout << "In Router_ONoC::rxProcess_PE(), received flit,
      // current_level_rx[0] = " << current_level_rx[0] << endl;

      // if a new flit is injected from local PE
      if (received_flit.src_id == local_id)
        power.networkInterface();

      cout << "In Router_ONoC::rxProcess_PE(), flowControl_signal["
           << received_flit.dst_id
           << "] = " << flowControl_signal[received_flit.dst_id] << endl;
    }
...
void Router_ONoC::rxProcess_centralRouter() {
  if (reset.read()) {
    ...
    for (int i = 1; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y + 1;
         i++) {
      ACK t;
      t.value = 0;
      t.src_id = local_id;
    ...
    for (int i = 0; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y;
         i++) {
    ...
    for (int i = 1; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y + 1;
      ...
      Flit received_flit = flit_rx[i].read();
      if ((req_rx[i].read() == 1 - current_level_rx[i]) &&
          ((received_flit.src_id != local_id) &&
           (received_flit.src_id + 1 == i)) &&
          (!rx_buffer[received_flit.src_id].IsFull())) {

        cout << "In Router_ONoC::rxProcess_centralRouter(), "
                "received_flit.src_id = "
             << received_flit.src_id
             << ", req_rx[i].read() = " << req_rx[i].read() << endl;
        ...
        if (received_flit.dst_id == local_id) {
          ...
          cout << "In Router_ONoC::rxProcess_centralRouter(), Push flit :"
               << received_flit << " into rx_buffer[" << received_flit.src_id
               << "]" << endl;

          rx_buffer[received_flit.src_id].Print("rx_buffer");
          cout << "In Router_ONoC::rxProcess_centralRouter(), "
                  "rx_buffer[received_flit.src_id].Size() = "
               << rx_buffer[received_flit.src_id].Size() << endl;
          ...
          if (received_flit.flit_type == FLIT_TYPE_TAIL) {
            ...
            opticalPower.addNewLaserTime(
                tmpPathInfo.Wavelength_type, src_mid, dst_mid, -1,
                (sc_time_stamp().to_double() / GlobalParams::clock_period_ps));
            // addNewLaserTime(int w_type, int src_id, int dst_id, int
            // start_time, int end_time);
            cout << "\nopticalPower.addNewLaserTime: local_id = " << local_id
                 << ", received_flit.src_id = " << received_flit.src_id
                 << ", received_flit.dst_id = " << received_flit.dst_id
                 << ", end_time = "
                 << sc_time_stamp().to_double() / GlobalParams::clock_period_ps
                 << endl;
          }
          ...
          string t =
              "\nIn Router_ONoC::rxProcess_centralRouter(), Error condition -> "
              "received_flit.dst_id != local_id.";
          TM_ASSERT(false, t);
          assert(false);
        }
        ...
        if (received_flit.flit_type == FLIT_TYPE_HEAD) { // ACK Laser on
          ...
          PathInfo tmpPathInfo = opticalPower.getPathInfo(src_mid, dst_mid);
          opticalPower.addNewLaserTime(
              tmpPathInfo.Wavelength_type, src_mid, dst_mid,
              (sc_time_stamp().to_double() / GlobalParams::clock_period_ps),
              -1);
          cout << "\nopticalPower.addNewLaserTime: local_id = " << local_id
               << ", i-1 = " << i - 1 << ", start_time = "
          ...
          TM_SIMBUFFERSTATUSLOG(
              34,
              static_cast<tm_task *>(received_flit.src_task)->getApp()->getId(),
              static_cast<tm_task *>(received_flit.src_task)->getId(),
              static_cast<tm_task *>(received_flit.dst_task)->getId(),
              " FULL cycles: " << fixed << setprecision(2)
                               << (buffer_full_counter[received_flit.src_id] /
                                   GlobalParams::clock_period_optical_ps));
          buffer_full_counter[received_flit.src_id] = 0.0;
        }

        if (received_flit.flit_type == FLIT_TYPE_TAIL) {
          TM_SIMOPTICALEND2ENDLATENCYLOG(
              34,
              static_cast<tm_task *>(received_flit.src_task)->getApp()->getId(),
              static_cast<tm_task *>(received_flit.src_task)->getId(),
              static_cast<tm_task *>(received_flit.dst_task)->getId(),
              "end receiving tail_flit at "
        ...
        if (received_flit.src_id == local_id)
          power.networkInterface();
      } else if ((req_rx[i].read() == 1 - current_level_rx[i]) &&
                 rx_buffer[received_flit.src_id].IsFull()) {
      ...
      if ((req_rx[i].read() == 1 - current_level_rx[i]) &&
        ...
        if (received_flit.dst_id == local_id) {
          if (received_flit.flit_type == FLIT_TYPE_FLOWCONTROL) {
            flowControl_signal[received_flit.src_id] =
                received_flit.flow_control;
            cout << "In Router_ONoC::rxProcess_centralRouter(), local_id = "
                 << local_id << ", received flowControl :" << received_flit
                 << endl;
            ...
            if (received_flit.flow_control == false) { // laser off
              ...
              PathInfo tmpPathInfo = opticalPower.getPathInfo(src_mid, dst_mid);
              opticalPower.addNewLaserTime(tmpPathInfo.Wavelength_type, src_mid,
                                           dst_mid, -1,
                                           (sc_time_stamp().to_double() /
            ...
            if (received_flit.flit_type == FLIT_TYPE_TAIL) {
              ...
              // start_time, int end_time);
              cout << "In Router_ONoC::rxProcess_centralRouter(), "
                      "addNewLaserTime, local_id = "
                   << local_id << ", received tail flit = " << received_flit
                   << ", Wavelength_type = " << tmpPathInfo.Wavelength_type
                   << endl;
            }
            if (flowControl_table[received_flit.src_id] == true &&
                (rx_buffer[received_flit.src_id].getFlowControlOFF() >=
                 rx_buffer[received_flit.src_id].GetMaxBufferSize() -
                     rx_buffer[received_flit.src_id].Size())) {
              Flit flit;
              double ccycle =
                  sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
              ...
              power.bufferRouterPush();
              cout << "\nIn Router_ONoC::rxProcess_centralRouter(), local_id = "
                   << local_id << " : insert flowControl=false, rx_buffer["
                   << received_flit.src_id
                   << "].Size() = " << rx_buffer[received_flit.src_id].Size()
                   << ", rx_buffer[" << received_flit.src_id
                   << "].getFlowControlOFF() = "
                   << rx_buffer[received_flit.src_id].getFlowControlOFF()
                   << endl;
              cout << "Insert flowControl_flit(" << flit << ") to tx_buffer["
                   << received_flit.src_id
                   << "], size = " << tx_buffer[received_flit.src_id].Size()
                   << endl;
            ...
            if (received_flit.flit_type == FLIT_TYPE_TAIL) {
              TM_SIMOPTICALEND2ENDLATENCYLOG(
                  34,
                  static_cast<tm_task *>(received_flit.src_task)
                      ->getApp()
                      ->getId(),
                  static_cast<tm_task *>(received_flit.src_task)->getId(),
                  static_cast<tm_task *>(received_flit.dst_task)->getId(),
  ...
  if (reset.read()) {
    ...
    for (int i = 1; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y + 1;
      ...
      if (!tx_buffer[i - 1].IsEmpty()) {
        ...
        // cout << "In Router_ONoC::txProcess_centralRouter(), router local_id =
        // " << local_id << ", current_level_tx["<<i<<"] = " <<
        // current_level_tx[i] << ", ack_tx["<<i<<"].read() = " <<
        // ack_tx[i].read().value << endl;

        if (current_level_tx[i] ==
            ack_tx[i].read().value) // current_level_tx[o] == ack_tx[o].read()
        {
          cout << "\nIn txProcess_centralRouter(), router local_id = "
               << local_id << endl;
          // if (GlobalParams::verbose_mode > VERBOSE_OFF)
          //  LOG << "Input[" << i << "] forward to Output[" << o << "], flit: "
          //  << flit << endl;
          if (o == DIRECTION_LOCAL_ONOC) {
            string t =
                "\nIn Router_ONoC::txProcess_centralRouter(), Error condition "
                "-> o == DIRECTION_LOCAL_ONOC.";
            TM_ASSERT(false, t);
            assert(false);
          } else
            cout << "Flit:" << flit
                 << " forward to direction: " << "CENTRAL_ROUTER (Serializer)"
                 << endl;
          ...
          if (o == DIRECTION_LOCAL_ONOC) {
            ...
            cout << "In Router_ONoC::txProcess_centralRouter(), to "
                    "central_router req_tx = "
                 << current_level_tx[i] << endl;
          }

          if (flit.flit_type == FLIT_TYPE_HEAD) {
            TM_SIMOPTICALEND2ENDLATENCYLOG(
                34, static_cast<tm_task *>(flit.src_task)->getApp()->getId(),
                static_cast<tm_task *>(flit.src_task)->getId(),
                static_cast<tm_task *>(flit.dst_task)->getId(),
                "start transmitting head_flit at "
                    << fixed << setprecision(2)
                    << (sc_time_stamp().to_double() /
                        GlobalParams::clock_period_optical_ps));
          }
          ...
          if (flit.flit_type == FLIT_TYPE_HEAD) {
            ...
            // start_time, int end_time);
            cout << "\nopticalPower.addNewLaserTime: local_id = " << local_id
                 << ", received_flit.src_id = " << flit.src_id
                 << ", received_flit.dst_id = " << flit.dst_id
                 << ", start_time = "
                 << sc_time_stamp().to_double() / GlobalParams::clock_period_ps
                 << endl;
          }
    ...
    for (int i = 1; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y + 1;
      ...
      if (!tx_buffer[i - 1].IsEmpty() && next_flit_state[i - 1] == 0) {
        ...
        // cout << "\nIn txProcess_centralRouter(), router local_id = " <<
        // local_id << ", flowControl_signal["<<i-1<<"] = " <<
        // flowControl_signal[i-1] << ", flit = " << flit << endl;

        if (flowControl_signal[i - 1] ||
            flit.flit_type == FLIT_TYPE_FLOWCONTROL) {
          ...
          if (flit.dst_id == local_id) {
            o = DIRECTION_LOCAL_ONOC;
            string t =
                "In Router_ONoC::txProcess_centralRouter(), Error "
                "condition->route_data.dst_id == local_id.";
            TM_ASSERT(false, t);
            assert(false);
          } else
            o = DIRECTION_CENTRAL_ROUTER;
          ...
          cout << "In txProcess_centralRouter(), to "
                  "central_router(Serializer),before current_level_tx[i] = 1 - "
                  "current_level_tx[i]; req_tx and current_level_tx["
               << i << "] = " << current_level_tx[i] << ", o = " << o << endl;
          current_level_tx[i] = 1 - current_level_tx[i];
          req_tx[i].write(current_level_tx[i]);
          cout << "In txProcess_centralRouter(), to central_router(Serializer) "
                  "req_tx and current_level_tx["
               << i << "] = " << current_level_tx[i] << endl;
          if (flit.flit_type == FLIT_TYPE_HEAD) {
            cout << "In txProcess_centralRouter(), flit.flit_type == "
                    "FLIT_TYPE_HEAD, flit = "
                 << flit << endl;
            TM_SIMOPTICALEND2ENDLATENCYLOG(
                34, static_cast<tm_task *>(flit.src_task)->getApp()->getId(),
...
void Router_ONoC::txProcess_PE() // reservation output port to local pe
{
  if (reset.read()) {
    // Clear outputs and indexes of transmitting protocol
    for (int i = 0; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y + 1;
         i++) {
    ...
    // buffer[i].deadlockCheck();
    for (int offset = 0;
         offset < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y + 1;
         offset++) {
      // cout << "In Router_ONoC::txProcess_PE(), print buffer: " << endl;
      // cout << "i = " << i ;
      // rx_buffer[i].Print("rx_buffer");
      int i = (start_from_port + offset) %
              (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y + 1);
      if (i == 0)
        i = 1;
      ...
      if (!rx_buffer[i - 1].IsEmpty()) {
        ...
        cout << "flit.dst_id = " << flit.dst_id << ", local_id = " << local_id
             << endl;
        if (flit.dst_id == local_id)
          o = DIRECTION_LOCAL_ONOC;
        else
        ...
        // cout << "In Router_ONoC::txProcess_PE(), router local_id = " <<
        // local_id << ", current_level_tx[o] = "<<current_level_tx[o]<<",
        // ack_tx_local = " << ack_tx_local << ", flit = " << flit << endl; cout
        // << "current_level_tx[o] = " << current_level_tx[o] << ",
        // ack_tx[o].read() = " << ack_tx_local.read() << endl;

        if (current_level_tx[o] ==
            ack_tx_local.read()) // current_level_tx[o] == ack_tx[o].read()
        {
          // if (GlobalParams::verbose_mode > VERBOSE_OFF)
          //  LOG << "Input[" << i << "] forward to Output[" << o << "], flit: "
          //  << flit << endl;
          cout << "in" << endl;
          if (o == DIRECTION_LOCAL_ONOC)
            cout << "Flit:" << flit << " forward to direction: " << "LOCAL"
                 << endl;
          else {
            cout << "Flit:" << flit
                 << " forward to direction: " << "CENTRAL_ROUTER" << endl;
            cout << "rx_buffer[" << i - 1
                 << "].size() = " << rx_buffer[i - 1].Size() << endl;
            rx_buffer[i - 1].Print("rx_buffer");
            string t =
                "\nIn Router_ONoC::txProcess_PE(), Error condition -> o != "
                "DIRECTION_LOCAL_ONOC.";
            TM_ASSERT(false, t);
            assert(false);
          }
          ...
          // else {
          // 	current_level_tx[o+flit.dst_id] = 1 -
          // current_level_tx[o+flit.dst_id];
          // 	req_tx[o].write(current_level_tx[o+flit.dst_id]);
          // 	cout << "In Router_ONoC::txProcess_PE(), to central_router
          // req_tx = " << current_level_tx[o+flit.dst_id] << endl;
          // }
          ...
          cout << "In Router_ONoC::txProcess_PE(), local_id =  " << local_id
               << ", flowControl_table[" << i - 1
               << "] = " << flowControl_table[i - 1] << ", after pop rx_buffer["
               << i - 1 << "]:" << endl;

          rx_buffer[i - 1].Print("rx_buffer");

          // cout << "In Router_ONoC::txProcess_PE(), router local_id = " <<
          // local_id << endl; cout << "req_tx[o].write: " <<
          // current_level_tx[o] << endl << endl;
          ...
          if (GlobalParams::network_protocol == ON_OFF_FLOWCONTROL &&
              flowControl_table[i - 1] == false &&
              (rx_buffer[i - 1].GetMaxBufferSize() - rx_buffer[i - 1].Size() >=
               rx_buffer[i - 1].getFlowControlON())) {
            Flit flit;
            double ccycle =
                sc_time_stamp().to_double() / GlobalParams::clock_period_ps;
            flit.src_id = local_id;
            ...
            cout << "\nIn Router_ONoC::txProcess_PE(), local_id = " << local_id
                 << " : insert flowControl=true, tx_buffer[" << i - 1
                 << "].Size() = " << tx_buffer[i - 1].Size() << endl;
            cout << "Insert flowControl_flit(" << flit << ") to tx_buffer["
                 << i - 1 << "]" << endl;
            cout << "rx_buffer[i-1].GetMaxBufferSize() = "
                 << rx_buffer[i - 1].GetMaxBufferSize()
                 << ", rx_buffer[i-1].Size() = " << rx_buffer[i - 1].Size()
                 << ", rx_buffer[i-1].getFlowControlON() = "
                 << rx_buffer[i - 1].getFlowControlON() << endl;
            cout << "rx_buffer[" << i - 1 << "] ->";
            rx_buffer[i - 1].Print("rx_buffer");
            cout << "tx_buffer[" << i - 1 << "] ->";
          ...
          start_from_port =
              ...
              (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y + 1);
          // cout << "In Router_ONoC::txProcess_PE(), local_id =  " << local_id
          // << ", i = " << i << ", start_from_port = " << start_from_port << ",
          // sending flit: " << flit <<"  req_tx[o].read() ="<<req_tx[o].read()
          // << endl;
          break;
        }
      }
...
void Router_ONoC::perCycleUpdate() {
  ...
  if (!reset.read()) {
    power.leakageRouter();
    for (int i = 0; i < GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y * 2;
         i++) {
      power.leakageBufferRouter();
      // power.leakageLinkRouter2Router();
    }
...
void Router_ONoC::configure(const int _id, const double _warm_up_time,
                            const unsigned int _max_buffer_size,
                            GlobalRoutingTable &grt) {
  local_id = _id;
  stats.configure(_id, _warm_up_time);
...
unsigned long Router_ONoC::getRoutedFlits() { return routed_flits; }
