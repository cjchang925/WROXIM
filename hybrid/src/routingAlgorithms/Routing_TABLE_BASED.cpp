...
RoutingAlgorithmsRegister Routing_TABLE_BASED::routingAlgorithmsRegister(
    "TABLE_BASED", getInstance());
...
vector<int> Routing_TABLE_BASED::route(Router *router,
                                       const RouteData &routeData) {
  Coord current = id2Coord(routeData.current_id);
  Coord destination = id2Coord(routeData.dst_id);

  AdmissibleOutputs ao = router->routing_table.getAdmissibleOutputs(
      routeData.dir_in, routeData.dst_id);

  if (ao.size() == 0) {
    LOG << "dir: " << routeData.dir_in << ", (" << current.x << "," << current.y
