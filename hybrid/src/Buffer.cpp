...
void Buffer::Disable() { true_buffer = false; }
...
unsigned int Buffer::GetMaxBufferSize() const { return max_buffer_size; }

bool Buffer::IsFull() const { return buffer.size() == max_buffer_size; }

bool Buffer::IsEmpty() const { return buffer.size() == 0; }

void Buffer::Drop(const Flit &flit) const { assert(false); }

void Buffer::Empty() const { assert(false); }
...
unsigned int Buffer::Size() const { return buffer.size(); }
