...
SC_MODULE(Router) {
  ...
  sc_in<bool>
      req_rx[DIRECTIONS + 2]; // The requests associated with the input channels
  sc_out<bool> ack_rx[DIRECTIONS + 2]; // The outgoing ack signals associated
                                       // with the input channels

  sc_out<Flit> flit_tx[DIRECTIONS + 2]; // The output channels
  sc_out<bool> req_tx[DIRECTIONS +
                      2]; // The requests associated with the output channels
  sc_in<bool> ack_tx[DIRECTIONS + 2]; // The outgoing ack signals associated
  ...
  bool current_level_rx[DIRECTIONS +
                        2]; // Current level for Alternating Bit Protocol (ABP)
  bool current_level_tx[DIRECTIONS +
                        2]; // Current level for Alternating Bit Protocol (ABP)
...
SC_MODULE(Router_ONoC) {
  ...
  sc_out<ACK>
      *ack_rx; // The outgoing ack signals associated with the input channels
  ...
  sc_in<ACK>
      *ack_tx; // The outgoing ack signals associated with the output channels
  ...
  SC_CTOR(Router_ONoC) {
    // cout << "In Router_ONoC constructor, before new flit_rx
    // port.-----------------" << endl;
    //* -----------
 ...
 private:
  ...
  // NoP_data getCurrentNoPData();
  // void NoP_report() const;
  // int NoPScore(const NoP_data & nop_data, const vector <int> & nop_channels)
  // const;
  int reflexDirection(int direction) const;
  int getNeighborId(int _id, int direction) const;
