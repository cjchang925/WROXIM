  ...
  while (!fin.eof()) {
    ...
    if (line[0] != '\0') {
      if (line[0] != '%') {
        ...
        int params = sscanf(line, "%d %d %lf %lf %d %d %d", &src, &dst, &pir,
        ...
        if (params >= 2) {
          ...
          if (params >= 4 && por >= 0 && por <= 1)
            ...
            communication.por =
                communication
                    .pir; // GlobalParams::probability_of_retransmission;
...
double GlobalTrafficTable::getCumulativePirPor(
    const int src_id, const int ccycle, const bool pir_not_por,
...
int GlobalTrafficTable::occurrencesAsSource(const int src_id) {
  ...
  for (unsigned int i = 0; i < traffic_table.size(); i++)
    if (traffic_table[i].src == src_id)
      count++;
  // cout << "GlobalTrafficTable::occurrencesAsSource -> count = " << count <<
  // endl;
  return count;
}
