...
vector<int> admissibleOutputsSet2Vector(const AdmissibleOutputs &ao) {
  ...
  for (AdmissibleOutputs::iterator i = ao.begin(); i != ao.end(); i++)
    dirs.push_back(oLinkId2Direction(*i));

  return dirs;
}

GlobalRoutingTable::GlobalRoutingTable() { valid = false; }
...
RoutingTableNode GlobalRoutingTable::getNodeRoutingTable(const int node_id) {
