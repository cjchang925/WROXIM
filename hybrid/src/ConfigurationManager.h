...
struct convert<HubConfig> {
  ...
  static bool decode(const Node &node, HubConfig &hubConfig) {
    hubConfig.attachedNodes = node["attachedNodes"].as<vector<int> >(
        GlobalParams::default_hub_configuration.attachedNodes);
    hubConfig.rxChannels = node["rxChannels"].as<vector<int> >(
        GlobalParams::default_hub_configuration.rxChannels);
    hubConfig.txChannels = node["txChannels"].as<vector<int> >(
        GlobalParams::default_hub_configuration.txChannels);
    hubConfig.toTileBufferSize = node["toTileBufferSize"].as<int>(
        GlobalParams::default_hub_configuration.toTileBufferSize);
    hubConfig.fromTileBufferSize = node["fromTileBufferSize"].as<int>(
        GlobalParams::default_hub_configuration.fromTileBufferSize);
    hubConfig.txBufferSize = node["txBufferSize"].as<int>(
        GlobalParams::default_hub_configuration.txBufferSize);
    hubConfig.rxBufferSize = node["rxBufferSize"].as<int>(
        GlobalParams::default_hub_configuration.rxBufferSize);
    return true;
  }
};
...
struct convert<ChannelConfig> {
  ...
  static bool decode(const Node &node, ChannelConfig &channelConfig) {
    channelConfig.ber = node["ber"].as<pair<double, double> >(
        GlobalParams::default_channel_configuration.ber);
    channelConfig.dataRate = node["dataRate"].as<int>(
        GlobalParams::default_channel_configuration.dataRate);
    channelConfig.macPolicy = node["mac_policy"].as<vector<string> >(
...
struct convert<RouterPowerConfig> {
  static bool decode(const Node &node, RouterPowerConfig &routerPowerConfig) {
    ...
    for (YAML::const_iterator crossbar_it = node["crossbar"].begin();
      ...
      routerPowerConfig.crossbar_pm[make_pair(v[0], v[1])] =
          make_pair(v[2], v[3]);
    }
    ...
    for (YAML::const_iterator routing_it = node["routing"].begin();
      ...
      routerPowerConfig.routing_algorithm_pm[routing_it->first.as<string>()] =
    ...
    for (YAML::const_iterator selection_it = node["selection"].begin();
      ...
      routerPowerConfig
          .selection_strategy_pm[selection_it->first.as<string>()] =
...
struct convert<PowerConfig> {
  static bool decode(const Node &node, PowerConfig &powerConfig) {
    ...
    powerConfig.pePowerConfig =
        node["PE"].as<PEPowerConfig>(); // Modified by LGGM
    return true;
  }
};
...
//     static bool decode(const Node& node, ArbitrationFlowControlConfig&
...
struct convert<OpticalPowerConfig> {
  static bool decode(const Node &node, OpticalPowerConfig &opticalPowerConfig) {
    ...
    opticalPowerConfig.serializerConfig =
        node["Serializer"].as<SerializerConfig>();
    opticalPowerConfig.driverConfig = node["Driver"].as<DriverConfig>();
    opticalPowerConfig.rxComparatorConfig =
        node["RxComparator"].as<RxComparatorConfig>();
    opticalPowerConfig.tiaConfig = node["TIA"].as<TIAConfig>();
    // opticalPowerConfig.arbitrationFlowControlConfig =
    // node["Arbitration_FlowControl"].as<ArbitrationFlowControlConfig>();
