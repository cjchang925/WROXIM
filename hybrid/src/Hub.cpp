...
int Hub::tile2Port(int id) { return tile2port_mapping[id]; }

int Hub::route(Flit &f) {
  for (vector<int>::size_type i = 0;
       i < GlobalParams::hub_configuration[local_id].attachedNodes.size();
       i++) {
    if (GlobalParams::hub_configuration[local_id].attachedNodes[i] ==
        f.dst_id) {
...
void Hub::txPowerManager() {
  for (unsigned int i = 0; i < txChannels.size(); i++) {
    ...
    } else { // abtx is empty and not reserved - turn off
...
void Hub::updateTxPower() {
  ...
  for (int i = 0; i < num_ports; i++) power.leakageBufferFromTile();
}
...
void Hub::txRadioProcessTokenPacket(int channel) {
  ...
  if (current_token_holder[channel]->read() == local_id) {
    if (!init[channel]->buffer_tx.IsEmpty()) {
      bool is_tail =
          init[channel]->buffer_tx.Front().flit_type == FLIT_TYPE_TAIL;
      ...
      if (is_tail) {
        LOG << "TOKEN_PACKET: tail sent, releasing token for channel "
            << channel << endl;
        flag[channel]->write(RELEASE_CHANNEL);
      }

    } else {
      // LOG << "TOKEN_PACKET: Buffer_tx empty, releasing token for channel " <<
      // channel << endl;
      flag[channel]->write(RELEASE_CHANNEL);
    }
  }
...
void Hub::txRadioProcessTokenHold(int channel) {
  ...
  if (current_token_holder[channel]->read() == local_id) {
    if (!init[channel]->buffer_tx.IsEmpty()) {
      // LOG << "Token holder for channel " << channel << " with not empty
      // buffer_tx" << endl;

      if (current_token_expiration[channel]->read() <
          flit_transmission_cycles[channel]) {
        // LOG << "TOKEN_HOLD policy: Not enough token expiration time for
        // sending channel " << channel << endl;
      } else {
        flag[channel]->write(HOLD_CHANNEL);
        LOG << "Starting transmission on channel " << channel << endl;
        init[channel]->start_request_event.notify();
      ...
      // LOG << "TOKEN_HOLD policy: nothing to transmit, holding token for
...
void Hub::txRadioProcessTokenMaxHold(int channel) {
  ...
  if (current_token_holder[channel]->read() == local_id) {
    if (!init[channel]->buffer_tx.IsEmpty()) {
      ...
      if (current_token_expiration[channel]->read() <
          flit_transmission_cycles[channel]) {
        // LOG << "TOKEN_MAX_HOLD: Not enough token expiration time, releasing
        // token for channel " << channel << endl;
        flag[channel]->write(RELEASE_CHANNEL);
      } else {
        flag[channel]->write(HOLD_CHANNEL);
        LOG << "Starting transmission on channel " << channel << endl;
      ...
      // LOG << "TOKEN_MAX_HOLD: Buffer_tx empty, releasing token for channel "
      // << channel << endl;
      flag[channel]->write(RELEASE_CHANNEL);
    }
  }
...
void Hub::antennaToTileProcess() {
  ...
  // The rxPowerManager must perform its checks before the flits are removed
  // from buffers
  updateRxPower();
  ...
  for (int i = 0; i < num_ports; i++) {
    if (!buffer_to_tile[i].IsEmpty()) {
      ...
      LOG << "Flit moved from buffer_to_tile[" << i << "] to signal flit_tx["
          << i << "] " << endl;
  ...
  for (unsigned int i = 0; i < rxChannels.size(); i++) {
    ...
    if (!(target[channel]->buffer_rx.IsEmpty())) {
      ...
      if (!buffer_to_tile[r[i]].IsFull()) {
        ...
        LOG << "Moving flit from buffer_rx to buffer_to_tile, port " << r[i]
            << endl;
    ...
    if (!buffer_from_tile[i].IsEmpty()) {
      ...
      if (flit.flit_type == FLIT_TYPE_HEAD) {
        ...
        if (tile2antenna_reservation_table.isAvailable(channel)) {
          ...
          // LOG << "Reservation:  wireless channel " << channel << " not
          // available ..." << endl;
        }
      }
    }
    ...
    if (!buffer_from_tile[i].IsEmpty()) {
      ...
      if (channel != NOT_RESERVED) {
        if (!(init[channel]->buffer_tx.IsFull())) {
          LOG << "Flit moved from buffer_from_tile[" << i << "] to buffer_tx["
              << channel << "] " << endl;
        ...
        // LOG << "Forwarding: No channel reserved for port direction " << i  <<
        // endl;
      }
    }
