...
std::multimap<int, tuple<int,int, vector<laser_time>>> OpticalPower::laser_time_unsorted;
...
std::multimap<int, tuple<int, vector<laser_time>>> OpticalPower::optical_tx_time_unsorted;
std::multimap<int, tuple<int, vector<laser_time>>> OpticalPower::optical_Rx_time_unsorted;
std::map<int, vector<tuple<int, int, int, set<int>>>> OpticalPower::optical_tx_intervals;
std::map<int, vector<tuple<int, int, int, set<int>>>> OpticalPower::optical_Rx_intervals;
std::map<int, double> OpticalPower::path_loss;

#define mW2J(mwatt) ((mwatt*1.0e-3)*GlobalParams::clock_period_ps*1.0e-12)
#define mW2J_clk2(mwatt) ((mwatt*1.0e-3)*GlobalParams::clock_period_optical_ps*1.0e-12)

void OpticalPower::parsePathData() {
    string file_path = "./path_input/" + GlobalParams::architecture + to_string(GlobalParams::mesh_dim_x*GlobalParams::mesh_dim_y) +".txt";
...
void OpticalPower::parseMasterSlaveData() {
    string file_path = "./MasterSlave/" + GlobalParams::architecture + 
...
void OpticalPower::parsePathLossFile() {
    string filename = "./power_input/" + GlobalParams::architecture + to_string(GlobalParams::mesh_dim_x*GlobalParams::mesh_dim_y) + ".txt";
    ...
    while (std::getline(infile, line)) {
        ...
        if (pathData.find(path_key) != pathData.end()) {
            ...
            if (path_loss.find(wavelength_type) == path_loss.end() || path_loss[wavelength_type] < loss_value) {
...
void OpticalPower::addNewTxTime(int chiplet_id, int w_type, int start_time, int end_time) {
    if (end_time == -1) {   // add new start_time
        ...
        optical_tx_time_unsorted.emplace(chiplet_id, make_tuple(w_type, vector<laser_time>{new_laser_time}));
        return;
    }
        ...
        cout << endl \
             << "******************" << endl \
             << "ABNORMAL SITUATION : (cycle=" << sc_time_stamp().to_double() / GlobalParams::clock_period_ps << ") , in OpticalPower::addNewLaserTime()." << endl \
             << "******************" << endl;
        assert(false);
    }
}
void OpticalPower::addNewRxTime(int chiplet_id, int w_type, int start_time, int end_time) {
    if (end_time == -1) {   // add new start_time
        laser_time new_laser_time{start_time, end_time};
        ...
        optical_Rx_time_unsorted.emplace(chiplet_id, make_tuple(w_type, vector<laser_time>{new_laser_time}));
        return;
    }
    else if (start_time == -1) {    // add new end_time
        auto range = optical_Rx_time_unsorted.equal_range(chiplet_id);
...
void OpticalPower::addNewLaserTime(int w_type, int src_id, int dst_id, int start_time, int end_time) {
    if (end_time == -1) {   // add new start_time
        laser_time new_laser_time{start_time, end_time};
        ...
        laser_time_unsorted.emplace(w_type, make_tuple(src_id, dst_id, vector<laser_time>{new_laser_time}));
        return;
    }
    else if (start_time == -1) {    // add new end_time
        auto range = laser_time_unsorted.equal_range(w_type);
...
void OpticalPower::calculateWavelengthIntervals() {
    ...
    for (const auto& entry : laser_time_unsorted) {
        ...
        combined_intervals[w_type].insert(combined_intervals[w_type].end(), times.begin(), times.end());
    }

    for (auto& [w_type, laser_intervals] : combined_intervals) {
        std::sort(laser_intervals.begin(), laser_intervals.end(), [](const laser_time& a, const laser_time& b) {
        ...
        for (auto& interval : laser_intervals) {
            if(interval.end_time == -1){
                interval.end_time = GlobalParams:: simulation_time + GlobalParams::reset_time;
            }
            if (merged_intervals.empty() || merged_intervals.back().end_time < interval.start_time) {
                merged_intervals.push_back(interval);
            } else {
                merged_intervals.back().end_time = std::max(merged_intervals.back().end_time, interval.end_time);
            }
        }
...
void OpticalPower::calculateWavelengthUsageTime() {
    for (const auto& [wavelength, laser_intervals] : wavelength_intervals) {
        ...
        cout << "Wavelength " << wavelength << " -> total usage time: " << total_time << endl;
    }
}
...
void OpticalPower::calculateLaserIntervals() {
    ...
    sort(events.begin(), events.end(), [](const tuple<int, int>& a, const tuple<int, int>& b) {
        if (get<0>(a) != get<0>(b)) {
    ...
    for (const auto& [time, change] : events) {
        if (prev_time != -1 && !active_wavelengths.empty()) {
            ...
            laser_intervals.emplace_back(prev_time, time, active_wavelengths.size(), active_wavelengths);
        }
...
void OpticalPower::calculateOpticalTxIntervals() {
    ...
    for (const auto& entry : optical_tx_time_unsorted) {
        ...
        combined_intervals[chiplet_id][w_type].insert(combined_intervals[chiplet_id][w_type].end(), times.begin(), times.end());
    }
    ...
    for (const auto& chiplet_entry : combined_intervals) {
        ...
        for (const auto& w_type_entry : w_type_map) {
            ...
            sort(merged_intervals.begin(), merged_intervals.end(), [](const laser_time& a, const laser_time& b) {
                return a.start_time < b.start_time;
            });
            ...
            for (auto& interval : merged_intervals) {
                ...
                if (final_intervals.empty() || final_intervals.back().end_time < interval.start_time) {
                    final_intervals.push_back(interval);
                } else {
                    final_intervals.back().end_time = max(final_intervals.back().end_time, interval.end_time);
                }
            }
        ...
        for (const auto& event : events) {
            ...
            if (prev_time != -1 && !active_wavelengths.empty()) {
                ...
                chiplet_intervals.emplace_back(prev_time, time, active_wavelengths.size(), active_wavelengths);
            }
...
double OpticalPower::getLaserEnergy()
{
    ...
    double wavelength_total_count = GlobalParams::mesh_dim_x*GlobalParams::mesh_dim_y;

    for (const auto& [start, end, count, wavelengths] : laser_intervals) {
        total_laser_energy += mW2J(GlobalParams::optical_power_configuration.laserConfig.active_mode * wavelength_total_count * count) * (end - start + 1);
    }
...
double OpticalPower::getLaserLossEnergy()
{
    ...
    //     double pathloss_power = 0.0;
    //     for (const auto& w : wavelengths){
    //         cout<< "wavelength: " << w << ", path_loss: " << path_loss[w] << endl;
    //         pathloss_power += pow(10, (path_loss[w]+Er)/10)/ (Ec*El);
    //     }
...
double OpticalPower::getTxEnergy() {
    ...
    for (const auto& chiplet_entry : optical_tx_intervals) {
        ...
        for (const auto& interval : intervals) {
            ...
            double energy = mW2J_clk2((GlobalParams::optical_power_configuration.driverConfig.active_mode * count) + (GlobalParams::optical_power_configuration.serializerConfig.active_mode * count) + (GlobalParams::optical_power_configuration.serializerConfig.idle_mode * (wavelength_total_count-count))) * (end - start + 1);
            total_tx_energy += energy;
            // cout << "Tx Chiplet ID: " << chiplet_id << ", Interval: [" << start << ", " << end << "], Count: " << count 
            //           << ", Energy: " << energy << " J\n";
        }
    }
...
double OpticalPower::getRxEnergy()
{
    ...
    for (const auto& chiplet_entry : optical_Rx_intervals) {
        ...
        for (const auto& interval : intervals) {
            ...
            double energy = mW2J_clk2((GlobalParams::optical_power_configuration.tiaConfig.active_mode * count)\
             + (GlobalParams::optical_power_configuration.rxComparatorConfig.active_mode * count) + \
            (GlobalParams::optical_power_configuration.rxComparatorConfig.idle_mode * (wavelength_total_count-count)))\
             * (end - start + 1);
            total_rx_energy += energy;
            // cout << "Rx Chiplet ID: " << chiplet_id << ", Interval: [" << start << ", " << end << "], Count: " << count 
            //           << ", Energy: " << energy << " J\n";
        }
    }
...
//     for (const auto& [start, end, count, wavelengths] : laser_intervals) {
//         total_ArbitrationFlowControl_energy += mW2J(GlobalParams::optical_power_configuration.arbitrationFlowControlConfig.active_mode * (count/wavelength_total_count) + GlobalParams::optical_power_configuration.arbitrationFlowControlConfig.idle_mode * ((wavelength_total_count-count)/wavelength_total_count)) * (end - start + 1);
//     }
//     return total_ArbitrationFlowControl_energy;
// }
double OpticalPower::getEOEEnergy()
{   
    return getTxEnergy() + getRxEnergy();
}

double OpticalPower::getMRREnergy()
{
    string file_path = "./MRR_info/MRR" + to_string(GlobalParams::mesh_dim_x*GlobalParams::mesh_dim_y) + ".txt";
    ifstream file(file_path);
    ...
    double chiplet_MRR_number = (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y * 2)\
     * (GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y);
    // cout << "\nMRR_number[" << GlobalParams::architecture << "]" << MRR_number[GlobalParams::architecture] << endl;
    ...
    for (const auto& group : grouped_entries) {
        ...
        for (const auto& t : entries) {
            ...
            for (const auto& time : times) {
                if (time.start_time <= 0 || (time.start_time >= time.end_time && time.end_time != -1)) {
                    out << endl \
                            << "******************" << endl \
                            << "ABNORMAL SITUATION : in OpticalPower::printLaserTime(). Laser's start_time <= 0 or Laser's start_time >= end_time" << endl \
                            << "time.start_time: " << time.start_time << ", time.end_time: " << time.end_time << endl \
                            << "******************" << endl;
                    assert(false);
                } 
                ...
                //     // out << endl \
                //     //         << "******************" << endl \
                //     //         << "Please check ABNORMAL SITUATION : in OpticalPower::printLaserTime(). Laser's end_time <= 0" << endl \
                //     //         << "time.start_time: " << time.start_time << ", time.end_time: " << time.end_time << endl \
                //     //         << "******************" << endl;
                // } 
                else {
...
void OpticalPower::printWavelengthIntervals(std::ostream& out)
{
    ...
    for (const auto& entry : wavelength_intervals) {
        ...
        out << "Wavelength " << w_type << ":\n";
        for (const auto& interval : laser_intervals) {
            out << "\tLaser On: " << interval.start_time << ", Off: " << interval.end_time << "\n";
        }
    }
}
