...
YAML::Node optical_power_config;
...
void loadConfiguration() {
  ...
  cout << "Loading optical power configurations from file \""
       << GlobalParams::optical_power_config_filename << "\"...";
  try {
    optical_power_config =
        YAML::LoadFile(GlobalParams::optical_power_config_filename);
    cout << " Done" << endl;
  } catch (YAML::BadFile &e) {
    cout << " Failed" << endl;
    cerr
        << "The specified YAML optical power configurations file was not found!"
        << endl;
    exit(0);
  }

  // Initialize global configuration parameters (can be overridden with
  ...
  GlobalParams::routing_table_filename =
      config["routing_table_filename"].as<string>();
  GlobalParams::selection_strategy = config["selection_strategy"].as<string>();
  GlobalParams::packet_injection_rate =
      config["packet_injection_rate"].as<double>();
  GlobalParams::probability_of_retransmission =
      config["probability_of_retransmission"].as<double>();
  GlobalParams::traffic_distribution =
      config["traffic_distribution"].as<string>();
  GlobalParams::traffic_table_filename =
      config["traffic_table_filename"].as<string>();
  GlobalParams::clock_period_ps = config["clock_period_ps"].as<int>();

  GlobalParams::clock_period_optical_ps =
      config["clock_period_optical_ps"].as<int>(); // WRONoC
  ...
  GlobalParams::max_volume_to_be_drained =
      config["max_volume_to_be_drained"].as<unsigned int>();
  ...
  // WRONoC
  GlobalParams::architecture = config["architecture"].as<string>();
  GlobalParams::network_protocol = config["network_protocol"].as<string>();
  GlobalParams::use_TxRx_PreciseTiming =
      config["use_TxRx_PreciseTiming"].as<bool>();
  ...
  GlobalParams::default_channel_configuration =
      config["Channels"]["defaults"].as<ChannelConfig>();

  for (YAML::const_iterator channels_it = config["Channels"].begin();
       channels_it != config["Channels"].end(); ++channels_it) {
    ...
    GlobalParams::channel_configuration[channel_id] =
        channel_config_node.as<ChannelConfig>();
  }

  GlobalParams::power_configuration = power_config["Energy"].as<PowerConfig>();
  GlobalParams::optical_power_configuration =
      optical_power_config["Power"].as<OpticalPowerConfig>();
}
...
void showHelp(char selfname[]) {
  cout << "Usage: " << selfname << " [options]" << endl
       ...
       << "\t" << TM_CMDLINE
       << "\tLoad the specified task mapping configuration file"
       << endl // Modified by LGGM
       << "\t" << TM_CMDLINE_LOG << "\tSpecifies the task mapping log file"
       << endl // Modified by LGGM
       ...
       << "\t-buffer_antenna N\tSet the depth of hub antenna buffers (RX/TX) "
          "[flits]"
       << endl
       ...
       << "\t\tTABLE_BASED FILENAME\tRouting Table Based routing algorithm "
       ...
       << "\t\tpareto on off r\tSelf-similar Pareto distribution with given "
          "real parameters (alfa-on alfa-off r)"
       << endl
       << "\t\tcustom R\tCustom distribution with given real probability of "
          "retransmission"
       << endl
       << "\t-traffic TYPE\tSet the spatial distribution of traffic to TYPE "
          "where TYPE is one of the following:"
       << endl
       << "\t\t\tThis parameter is overrided if the cmd " << TM_CMDLINE
       << " is used." << endl
       << "\t\trandom\t\tRandom traffic distribution" << endl
       << "\t\tlocal L\t\tRandom traffic with a fraction L (0..1) of packets "
          "having a destination connected to the local hub, i.e. not using "
          "wireless"
       << endl
       ...
       << "\t\ttable FILENAME\tTraffic Table Based traffic distribution with "
          "table in the specified file"
       << endl
       << "\t-hs ID P\tAdd node ID to hotspot nodes, with percentage P (0..1) "
          "(Only for 'random' traffic)"
       << endl
       ...
       << "\t-volume N\tStop the simulation when either the maximum number of "
          "cycles has been reached or N flits have"
       << endl
       << "\t\t\tbeen delivered" << endl
       << "\t-sim N\t\tRun for the specified simulation time [cycles]" << endl
       << endl
       << "If you find this program useful please don't forget to mention in "
          "your paper Maurizio Palesi <maurizio.palesi@unikore.it>"
       << endl
       << "If you find this program useless please feel free to complain with "
          "Davide Patti <davide.patti@dieei.unict.it>"
       << endl
       << "If you want to send money please feel free to PayPal to Fabrizio "
          "Fazzino <fabrizio@fazzino.it>"
       << endl
       << "and if need to solve any other problem of your life please contact "
          "Turi Monteleone <salvatore.monteleone@dieei.unict.it>"
       << endl;
}
...
void showConfig() {
  cout << "Using the following configuration: " << endl
       ...
       << "- traffic_distribution = " << GlobalParams::traffic_distribution
       << endl
    ...
    for (int i = 1; i < arg_num; i++) {
      if (!strcmp(arg_vet[i], "-verbose"))
        ...
        if (!strcmp(traffic, "random"))
          ...
          GlobalParams::traffic_distribution = TRAFFIC_BUTTERFLY;
      ...
      else if (!strcmp(arg_vet[i], "-architecture")) // Added by JengDe
      {
        char *arch = arg_vet[++i];
        if (!strcmp(arch, "MESH"))
          GlobalParams::architecture = ARCH_MESH;
        else if (!strcmp(arch, "LAMBDA"))
          GlobalParams::architecture = ARCH_LAMBDA;
        else if (!strcmp(arch, "GWOR"))
          GlobalParams::architecture = ARCH_GWOR;
        else if (!strcmp(arch, "LIGHT"))
          GlobalParams::architecture = ARCH_LIGHT;
        else if (!strcmp(arch, "SNAKE"))
          GlobalParams::architecture = ARCH_SNAKE;
        else
          assert(false);
      } else if (!strcmp(arg_vet[i], "-network_protocol")) // Added by JengDe
      {
        char *arch = arg_vet[++i];
        if (!strcmp(arch, "ABP"))
          GlobalParams::network_protocol = ALTERNETING_BIT_PROTOCOL;
        else if (!strcmp(arch, "ON_OFF_FLOWCONTROL"))
          GlobalParams::network_protocol = ON_OFF_FLOWCONTROL;
        else
          assert(false);
      } else if (!strcmp(arg_vet[i],
                         "-use_TxRx_PreciseTiming")) // Added by JengDe
      {
        GlobalParams::use_TxRx_PreciseTiming = true;
      } else if (!strcmp(arg_vet[i], "-config") ||
                 !strcmp(arg_vet[i], "-power") ||
                 !strcmp(arg_vet[i], TM_CMDLINE) ||
                 !strcmp(arg_vet[i], TM_CMDLINE_LOG)) // Modified by LGGM
...
// Added by LGGM
// *************
void overrideTrafficConfiguration(void) {
  GlobalParams::traffic_distribution = TRAFFIC_TASKMAPPING;
  cout << "The traffic distribution has been changed to "
...
void configure(int arg_num, char *arg_vet[]) {
  ...
  bool optical_power_config_found = false; // Added by JengDe
  bool task_mapping_found = false;         // Added by LGGM
  bool task_mapping_log_found = false;     // Added by LGGM
  ...
  // ****************
  for (int i = 1; i < arg_num; i++) {
    if (!strcmp(arg_vet[i], "-optical_power") &&
        i < (arg_num - 1)) { // Modified by LGGM
      GlobalParams::optical_power_config_filename = arg_vet[++i];
      optical_power_config_found = true;
      break;
    }
  }

  if (!optical_power_config_found) {
    std::ifstream infile(OPTICAL_POWER_CONFIG_FILENAME);
    if (infile.good())
      GlobalParams::optical_power_config_filename =
          OPTICAL_POWER_CONFIG_FILENAME;
    else {
      cerr << "No YAML optical power configurations file found!\n Use "
              "-optical_power to load examples from config_examples folder"
           << endl;
      exit(0);
    }
  }
  ...
  // *************
  for (int i = 1; i < arg_num; i++) {
    if (!strcmp(arg_vet[i], TM_CMDLINE) && i < (arg_num - 1)) {
      GlobalParams::taskmapping_filename = arg_vet[++i];
  ...
  if (task_mapping_found) {
    ifstream taskmapping_file(GlobalParams::taskmapping_filename.c_str());
    if (!taskmapping_file.is_open()) {
      cerr << "The TaskMapping file " << GlobalParams::taskmapping_filename
           << " could not be opened!" << endl;
      exit(0);
    } else {
      taskmapping_file.close();
      overrideTrafficConfiguration();

      for (int i = 1; i < arg_num; i++) {
        if (!strcmp(arg_vet[i], TM_CMDLINE_LOG) && i < (arg_num - 1)) {
          GlobalParams::taskmappinglog_filename = arg_vet[++i];
          task_mapping_log_found = true;
          break;
        }
      }

      if (task_mapping_log_found) {
        ofstream taskmappinglog_file(
            GlobalParams::taskmappinglog_filename.c_str());
        ...
        GlobalParams::taskmappinglog_filename = TM_NOT_FILE;
      }
    }
  } else {
    GlobalParams::taskmapping_filename = TM_NOT_FILE;
  }
  // *************
