...
void loadConfiguration()
{
    ...
    cout << "Loading optical power configurations from file \"" << GlobalParams::optical_power_config_filename << "\"...";
    try
    {
        optical_power_config = YAML::LoadFile(GlobalParams::optical_power_config_filename);
        cout << " Done" << endl;
    }
    catch (YAML::BadFile &e)
    {
        cout << " Failed" << endl;
        cerr << "The specified YAML optical power configurations file was not found!" << endl;
        exit(0);
    }
    ...
    GlobalParams::clock_period_optical_ps = config["clock_period_optical_ps"].as<int>(); // WRONoC
    ...
    GlobalParams::use_TxRx_PreciseTiming = config["use_TxRx_PreciseTiming"].as<bool>();
    ...
    GlobalParams::optical_power_configuration = optical_power_config["Power"].as<OpticalPowerConfig>();
}
...
void showHelp(char selfname[])
{
    cout << "Usage: " << selfname << " [options]" << endl
         ...
         << "\t" << TM_CMDLINE << "\tLoad the specified task mapping configuration file" << endl // Modified by LGGM
         << "\t" << TM_CMDLINE_LOG << "\tSpecifies the task mapping log file" << endl            // Modified by LGGM
         ...
         << "\t\t\tThis parameter is overrided if the cmd " << TM_CMDLINE << " is used." << endl
...
void checkConfiguration()
{
    ...
    for (unsigned int i = 0; i < GlobalParams::hotspots.size(); i++)
    {
        ...
        {
            cerr << "Error: hotspot percentage must be in the interval [0,1]"
                 << endl;
            exit(1);
        }
    }
...
{
    if (arg_num == 1)
        cout << "Running with default parameters (use '-help' option to see how to override them)"
             << endl;
    else
    {
        for (int i = 1; i < arg_num; i++)
        {
            ...
            else if (!strcmp(arg_vet[i], "-use_TxRx_PreciseTiming")) // Added by JengDe
            {
                GlobalParams::use_TxRx_PreciseTiming = true;
            }
            else if (!strcmp(arg_vet[i], "-config") || !strcmp(arg_vet[i], "-power") || !strcmp(arg_vet[i], TM_CMDLINE) || !strcmp(arg_vet[i], TM_CMDLINE_LOG)) // Modified by LGGM
...
void overrideTrafficConfiguration(void)
{
    ...
    cout << "The traffic distribution has been changed to " << GlobalParams::traffic_distribution << ". " << endl;
}
// *************
...
void configure(int arg_num, char *arg_vet[])
{
    ...
    for (int i = 1; i < arg_num; i++)
    {
        if (!strcmp(arg_vet[i], "-config") && i < (arg_num - 1))
        { // Modified by LGGM
    ...
    for (int i = 1; i < arg_num; i++)
    {
        if (!strcmp(arg_vet[i], "-power") && i < (arg_num - 1))
        { // Modified by LGGM
    ...
    for (int i = 1; i < arg_num; i++)
    {
        if (!strcmp(arg_vet[i], "-optical_power") && i < (arg_num - 1))
        { // Modified by LGGM
    ...
    if (!optical_power_config_found)
    {
        ...
        if (infile.good())
            GlobalParams::optical_power_config_filename = OPTICAL_POWER_CONFIG_FILENAME;
        else
        {
            cerr << "No YAML optical power configurations file found!\n Use -optical_power to load examples from config_examples folder" << endl;
            exit(0);
        }
    }
    // ****************
    ...
    if (task_mapping_found)
    {
        ...
        {
            cerr << "The TaskMapping file " << GlobalParams::taskmapping_filename << " could not be opened!" << endl;
            exit(0);
        }
        else
        {
            ...
            {
                ofstream taskmappinglog_file(GlobalParams::taskmappinglog_filename.c_str());
