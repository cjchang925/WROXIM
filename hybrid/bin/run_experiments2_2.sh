...
for ARCH in "LIGHT" "SNAKE"; do
    for tg_idx in "aes" "fft" "mmul";do
        # echo "ARCH = MESH, buffer_depth = ${buffer_depth}, tg_idx = ${tg_idx}"
        # ./noxim -config ../tm_examples/01-16-nownoc.yaml -architecture ${ARCH} -buffer ${buffer_depth} -sim ${sim_time} -taskmapping ../tm_examples/experiments2/tg_${tg_idx}.map -taskmappinglog ../tm_log/tgff/tg_${tg_idx}/MESH/tg_${tg_idx}_${ARCH}_b${buffer_depth}.log > ../tm_log/tmp_output.log
        for buffer_depth in "8" "16" "32" "64";do
            ...
            for network_protocol in "ABP" "ON_OFF_FLOWCONTROL"; do
                ...
                ./noxim -config ../tm_examples/01-16-nownoc.yaml -architecture ${ARCH} -network_protocol ${network_protocol} -buffer ${buffer_depth} -sim ${sim_time} -taskmapping ../tm_examples/experiments2/tg_${tg_idx}.map -taskmappinglog ../tm_log/tgff/tg_${tg_idx}/${ARCH}/tg_${tg_idx}_${ARCH}_${network_protocol}_b${buffer_depth}.log > ../tm_log/tmp_output1.log
            done
        done
    done
done
